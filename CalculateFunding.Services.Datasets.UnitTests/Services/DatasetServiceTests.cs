...
namespace CalculateFunding.Services.Datasets.Services
{
    ...
    public class DatasetServiceTests : DatasetServiceTestsBase
    {
        ...
        public async Task CreateNewDataset_GivenValidModel_ReturnsOKResult()
        {
            ...
            blobClient
                .GetBlobSasUrl(Arg.Any<string>(), Arg.Any<DateTimeOffset>(), Arg.Any<BlobSasPermissions>())
        ...
        public async Task DownloadOriginalDatasetUploadFile_GivenVersionIsSpecifiedAndVersionExists_ReturnsOKResult()
        {
            ...
            blobClient
                .GetBlobSasUrl(blobName, Arg.Any<DateTimeOffset>(), Arg.Any<BlobSasPermissions>())
                .Returns(blobUrl);
        ...
        public async Task UploadRawDatasetFile_GivenNewFile_ReturnsOkResult()
        {
            ...
            blobClient.GetBlobReferenceFromServerAsync($"{datasetMetadataViewModelRaw.DatasetId}/v1/file.uploaded.xls")
                .Returns(blob);
            ...
            await blobClient
                .Received()
                .UploadAsync(Arg.Is(blob), Arg.Any<Stream>());

            await blobClient
                .Received()
                .AddMetadataAsync(Arg.Is(blob), Arg.Is<IDictionary<string,string>>(m =>
                    m["datadefinitionid"] == datasetMetadataViewModelRaw.DataDefinitionId &&
                    m["datasetid"] == datasetMetadataViewModelRaw.DatasetId &&
                    m["authorid"] == datasetMetadataViewModelRaw.AuthorId &&
                    m["authorname"] == datasetMetadataViewModelRaw.AuthorName &&
                    m["name"] == datasetMetadataViewModelRaw.Name &&
                    m["description"] == "<Null>" &&
                    m["fundingstreamid"] == datasetMetadataViewModelRaw.FundingStreamId &&
                    m["converterwizard"] == datasetMetadataViewModelRaw.ConverterEligible.ToString()
                ));
        }
        ...
        public async Task UploadDatasetFile_GivenNewFile_ReturnsOkResult()
        {
            ...
            blobClient.GetBlobReferenceFromServerAsync($"{datasetMetadataViewModel.DatasetId}/v1/file.uploaded.xls")
                .Returns(blob);
            ...
            await blobClient
                .Received()
                .AddMetadataAsync(Arg.Is(blob), Arg.Is<IDictionary<string,string>>(m =>
                    m["datadefinitionid"] == datasetMetadataViewModel.DataDefinitionId &&
                    m["datasetid"] == datasetMetadataViewModel.DatasetId &&
                    m["authorid"] == datasetMetadataViewModel.AuthorId &&
                    m["authorname"] == datasetMetadataViewModel.AuthorName &&
                    m["name"] == datasetMetadataViewModel.Name &&
                    m["description"] == "<Null>" &&
                    m["fundingstreamid"] == datasetMetadataViewModel.FundingStreamId &&
                    m["converterwizard"] == datasetMetadataViewModel.ConverterEligible.ToString()
                ));
        }
        ...
        public void UpdateDatasetAndVersionDefinitionName_GivenUpdatingCosmosCausesException_LogsAndThrowsException()
        {
            ...
            datasetRepository
                .GetDatasetsBySQLQuery(Arg.Any<Expression<Func<EntityModel.Dataset, bool>>>())
                .Returns(datasets);

            datasetRepository
                .When(x => x.SaveDatasets(Arg.Any<IEnumerable<Dataset>>(), true))
        ...
        public async Task DeleteDatasets_Deletes_Dependencies_Using_Correct_SpecificationId_And_DeletionType(string specificationId, DeletionType deletionType)
        {
            ...
            properties.Add("deletion-type", (int)deletionType);
