                        ...
                        case "source":
                            {
                                ...
                                var destCredential = new StorageSharedKeyCredential(_.destinationContainer.AccountName, _.destinationContainer.AccountKey);
                                var destServiceClient = new BlobServiceClient(new Uri($"https://{_.destinationContainer.AccountName}.blob.core.windows.net"), destCredential);
                                var destContainerClient = destServiceClient.GetBlobContainerClient(_.destinationContainer.Name);

                                var sourceCredential = new StorageSharedKeyCredential(_.sourceContainer.AccountName, _.sourceContainer.AccountKey);
                                var sourceServiceClient = new BlobServiceClient(new Uri($"https://{_.sourceContainer.AccountName}.blob.core.windows.net"), sourceCredential);
                                var sourceContainerClient = sourceServiceClient.GetBlobContainerClient(_.sourceContainer.Name);

                                IEnumerable<Task> containerBlobs = _.sourceContainer.Blobs.ToList().Select(async (blob) =>
                                {
                                    var sourceBlobClient = sourceContainerClient.GetBlobClient(blob);
                                    if (await sourceBlobClient.ExistsAsync())
                                    {
                                        var destBlobClient = destContainerClient.GetBlobClient(blob);

                                        var sasBuilder = new BlobSasBuilder
                                        {
                                            BlobContainerName = sourceContainerClient.Name,
                                            BlobName = blob,
                                            Resource = "b",
                                        ...
                                        Uri sourceBlobSasUri;
                                        try
                                        {
                                            sourceBlobSasUri = sourceBlobClient.GenerateSasUri(sasBuilder);
                                        }
                                        catch
                                        {
                                            var sasQuery = sasBuilder.ToSasQueryParameters(sourceCredential).ToString();
                                            sourceBlobSasUri = new Uri($"{sourceBlobClient.Uri}?{sasQuery}");
                                        }

                                        await destBlobClient.StartCopyFromUriAsync(sourceBlobSasUri);
                                    }
                                });
                        ...
                        case "datasets":
                            {
                                if (datasetBlobName != null)
                                {
                                    ...
                                    var sourceBlobClient = sourceContainerClient.GetBlobClient(datasetBlobName);
                                    if (await sourceBlobClient.ExistsAsync())
                                    {
                                        var destBlobClient = destContainerClient.GetBlobClient(datasetBlobName);

                                        var sasBuilder = new BlobSasBuilder
                                        {
                                            BlobContainerName = sourceContainerClient.Name,
                                            BlobName = datasetBlobName,
                                            Resource = "b",
                                            StartsOn = DateTimeOffset.UtcNow.AddMinutes(-15),
                                            ExpiresOn = DateTimeOffset.UtcNow.AddDays(7)
                                        };
