...
using CalcResultModel = CalculateFunding.Common.ApiClient.Results.Models;

namespace CalculateFunding.Services.Results.Repositories
{
    public class CosmosCalculationResultsRepository : ICalculationResultsRepository, IHealthChecker
    {
        private readonly ICosmosRepository _cosmosRepository;
        private readonly EngineSettings _engineSettings;

        public CosmosCalculationResultsRepository(ICosmosRepository cosmosRepository, EngineSettings engineSettings)
        {
        ...
        public Task<ServiceHealth> IsHealthOk()
        {
            ...
            {
                Name = nameof(CosmosCalculationResultsRepository)
            };
        ...
        public async Task<IEnumerable<CalcResultModel.AggregateCalculationResults>> GetAggregateCalculationResults(string specificationId, IEnumerable<string> calculationIds)
        {
            CosmosDbQuery query = new CosmosDbQuery
            {
                QueryText = $@"SELECT 
	                            r.calculation.id  AS calculationId,
                                SUM(r[""value""]) AS sumValue,
                                AVG(r[""value""]) AS avgValue,
                                MIN(r[""value""]) AS minValue,
                                MAX(r[""value""]) AS maxValue
                            FROM c
                            JOIN r IN c.content.calcResults
                            WHERE c.content.specificationId = @specificationId 
                                AND IS_DEFINED(r[""value""]) AND r[""value""] != null
                                AND ARRAY_CONTAINS(@calculationIds,  r.calculation.id)
                            GROUP BY r.calculation.id ",
                Parameters = new[]
                {
                    new CosmosDbQueryParameter("@specificationId", specificationId),
                    new CosmosDbQueryParameter("@calculationIds", calculationIds.ToArray())
                }
            };

            return (await _cosmosRepository.DynamicQuery(query))
                .Select(_ => new CalcResultModel.AggregateCalculationResults()
                {
                    CalculationId= (string)_.calculationId,
                    SumValue = _.sumValue == null ? 0 : (decimal)_.sumValue,
                    AvgValue = _.avgValue == null ? 0 : (decimal)_.avgValue,
                    MaxValue = _.maxValue == null ? 0 : (decimal)_.maxValue,
                    MinValue = _.minValue == null ? 0 : (decimal)_.minValue
                });
        }
    }
}
