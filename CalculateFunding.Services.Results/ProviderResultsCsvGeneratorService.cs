...
namespace CalculateFunding.Services.Results
{
    public class ProviderResultsCsvGeneratorService : JobProcessingService, IProviderResultsCsvGeneratorService, IHealthChecker
    {
        ...
        public override async Task Process(ServiceBusReceivedMessage message)
        {
            ...
            SdkBlobClient blob = await _blobClient.GetBlobReferenceFromServerAsync($"calculation-results-{specificationId}.csv");

            // Avoid making an SDK network call here during unit tests by not calling SetHttpHeadersAsync
            // which would attempt to contact the configured blob endpoint on the returned SdkBlobClient.
            // Instead, set metadata via the IBlobClient wrapper during upload which is mocked in tests.
