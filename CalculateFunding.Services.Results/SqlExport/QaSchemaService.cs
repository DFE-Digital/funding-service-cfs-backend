...
namespace CalculateFunding.Services.Results.SqlExport
{
    public class QaSchemaService : IQaSchemaService
    {
            ...
            IEnumerable<Calculation> templateCalculations)
        {
            ...
            {
                string name = $"Calc_{templateCalculation.TemplateCalculationId}_{_sqlNames.GenerateIdentifier(templateCalculation.Name)}";
                // Story 272723 - Trim to max 128 characters
                var trimName = TrimName(name);
            ...
            IEnumerable<CalcsApiCalculation> calculations)
        {
            ...
            {
                string name = $"Add_{_sqlNames.GenerateIdentifier(calculation.Name)}";
                // Bug 276185 - Trim to max 128 characters
                var trimName = TrimName(name);
                fields.Add(new SqlColumnDefinition
                {
                    Name = trimName,
