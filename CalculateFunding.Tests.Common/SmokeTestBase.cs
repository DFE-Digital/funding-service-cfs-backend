...
namespace CalculateFunding.Tests.Common
{
    public class SmokeTestBase
    {
        ...
        public async Task<SmokeResponse> RunSmokeTest(string queueName,
            Func<ServiceBusReceivedMessage, Task> action,
            ...
            bool useSession)
        {
            ...
            try
            {
                ...
                if (IsDevelopment)
                {
                    ...
                    messageProperties.Add("smoketest", smokeResponsesFromFunction?.FirstOrDefault(_ => _ == uniqueId));
                    ...
                    {
                        await Task.FromResult(msg.ApplicationProperties["smoketest"].Equals(uniqueId));
                    }
                    : action;
        ...
        private static IMessengerService AddServiceBus(IConfiguration configuration, string serviceName)
        {
            if (IsDevelopment)
            {
                CalculateFunding.Common.ServiceBus.StorageQueueManager queueClient = new CalculateFunding.Common.ServiceBus.StorageQueueManager("UseDevelopmentStorage=true");
