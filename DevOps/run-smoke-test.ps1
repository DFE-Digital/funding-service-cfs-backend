...
# Set TLS protocol based on service name
if ($ServiceName -eq "external") {
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    Write-Host "Using TLS 1.2 for external service"
}
else {
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls13
    Write-Host "Using TLS 1.3 for other services"
}
