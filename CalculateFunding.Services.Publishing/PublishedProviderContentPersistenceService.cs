...
namespace CalculateFunding.Services.Publishing
{
    public class PublishedProviderContentPersistenceService : IPublishedProviderContentPersistenceService
    {
        ...
        // after removing  blobs code for V3 we need to Indexing For Published Provider 
        public async Task IndexingForPublishedProvider(IEnumerable<PublishedProvider> publishedProvidersToUpdate, bool publishAll = false)
        {
            _logger.Information("Index for Published Provider");
            List<Task> allTasks = new List<Task>();
            SemaphoreSlim throttler = new SemaphoreSlim(initialCount: _publishingEngineOptions.SavePublishedProviderContentsConcurrencyCount);
            foreach (PublishedProvider provider in publishedProvidersToUpdate)
            {
            ...
            await TaskHelper.WhenAllAndThrow(allTasks.ToArray());
            _logger.Information("Index for Published Provider is Complated");
        }
    }
}
