...
namespace CalculateFunding.Services.Publishing.Repositories
{
    public class PublishedFundingRepository : IPublishedFundingRepository
    {
        ...
        private const int BatchProcessingSize = 5000;
        ...
        public async Task<IEnumerable<KeyValuePair<string, string>>> GetPublishedProviderIdsForApproval(string fundingStreamId, string fundingPeriodId, string[] publishedProviderIds = null)
        {
            ...
            IEnumerable<string[]> publishedProviderIdBatches = publishedProviderIds == null
                ? new[] { Array.Empty<string>() }
                : publishedProviderIds
                  .Select((id, index) => new { id, index })
                  .GroupBy(x => x.index / BatchProcessingSize)
                  .Select(g => g.Select(x => x.id).ToArray());

            String queryTextBuilder = @"
            ...
            {
                foreach (string[] publishedProviderId in publishedProviderIdBatches)
                {
                    string batchQueryTextBuilder = queryTextBuilder;
                    string publishedProviderIdQueryText = string.Join(',', publishedProviderId.Select((_, index) => $"@publishedProviderId_{index}"));
                    batchQueryTextBuilder = batchQueryTextBuilder + $" AND c.content.publishedProviderId IN ("+ publishedProviderIdQueryText +")";

                    cosmosDbQueryParameters.AddRange(publishedProviderId.Select((_, index) => new CosmosDbQueryParameter($"@publishedProviderId_{index}", _)));

                    CosmosDbQuery cosmosDbQuery = new CosmosDbQuery
                    {
                        QueryText = batchQueryTextBuilder.ToString(),
                        Parameters = cosmosDbQueryParameters
                    };

                    IEnumerable<dynamic> queryResults = await _repository
                     .DynamicQuery(cosmosDbQuery , itemsPerPage);

                    foreach (dynamic item in queryResults)
                    {
                        results.TryAdd((string)item.id, (string)item.partitionKey);
                    }
                }
            }
            else
            ...
            string monthYearOpened = null)
        {
            ...
            if (monthYearOpened.IsNotNullOrWhitespace() && !monthYearOpened.Equals(OtherMonthYearOpened))
            {
        ...
        public async Task<IEnumerable<KeyValuePair<string, string>>> GetPublishedReleasedProviderIds(string fundingStreamId, string specificationId)
        {
            Guard.IsNullOrWhiteSpace(fundingStreamId, nameof(fundingStreamId));
            Guard.IsNullOrWhiteSpace(specificationId, nameof(specificationId));
