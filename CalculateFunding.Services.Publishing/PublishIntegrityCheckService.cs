...
namespace CalculateFunding.Services.Publishing
{
    public class PublishIntegrityCheckService : JobProcessingService, IPublishIntegrityCheckService
    {
            ...
            TemplateMapping templateMapping, bool publishAll)
        {
            ...
            if (fundingConfiguration != null && !fundingConfiguration.OrganisationGroupings.Any())
            {
                // As per UD:131043 , we are no longer uloading the documents to the v3 blobs. So reindexing should be done as a last step
                await _publishedProviderContentsPersistenceService.IndexingForPublishedProvider(selectedPublishedProviders);
                _logger.Information($"Finished saving published provider");
            }
            else
            {
        ...
        private static IEnumerable<string> SetBatchProviders(ServiceBusReceivedMessage message)
        {
            IEnumerable<string> batchProviders = null;

            if (message.ApplicationProperties.ContainsKey("providers-batch"))
            {
                batchProviders = message.ApplicationProperties["providers-batch"].ToString().AsPoco<IEnumerable<string>>();
            }

            return batchProviders;
        }

        private static bool SetPublishAll(ServiceBusReceivedMessage message)
        {
            bool publishAll = false;
            if (message.ApplicationProperties.ContainsKey("publish-all"))
            {
                publishAll = bool.Parse(message.ApplicationProperties["publish-all"].ToString() ?? string.Empty);
            }
