...
namespace CalculateFunding.Services.Publishing
{
    public class PublishedProviderVersionService : IPublishedProviderVersionService, IHealthChecker
    {
        ...
        public async Task<IActionResult> GetPublishedProviderVersionBody(string publishedProviderVersionId)
        {
            ...
            try
            {
                SdkBlobClient blob = await _blobClientPolicy.ExecuteAsync(() => _blobClient.GetBlobReferenceFromServerAsync(blobName));
            ...
            string specificationId)
        {
            ...
            try
            {
                // Some in-memory or test blob client implementations can have timing/race issues
                // when creating a blob reference. Retry a small number of times to reduce
                // intermittent failures seen in acceptance tests.
                SdkBlobClient blob = null;

                const int maxAttempts = 6;
                for (int attempt = 0; attempt < maxAttempts; attempt++)
                {
                    blob = await _blobClient.GetBlobReferenceFromServerAsync(blobName);
                ...
                if (blob == null)
                {
                    throw new Exception($"Failed to get blob reference for '{blobName}'");
                }
        ...
        private async Task UploadBlob(SdkBlobClient blob, string contents, IDictionary<string, string> metadata)
        {
