...
namespace CalculateFunding.Services.Publishing.Comparers
{
    public class ProviderComparer : IEqualityComparer<Provider>
    {
        ...
        //This property will have value only when enableRestrictedProviderMetaData flag is true.
        private IEnumerable<string> _limitedvariances = Enumerable.Empty<string>();

        public IEnumerable<string> limitedVariances
        {
            get => _limitedvariances;
            set => _limitedvariances = value ?? Enumerable.Empty<string>();
        }
        ...
        public bool Equals(Provider x, Provider y)
        {
            ...
            {
                if(_limitedvariances.Any() && !_limitedvariances.Contains(property.Name))
                {
                    continue;
                }
