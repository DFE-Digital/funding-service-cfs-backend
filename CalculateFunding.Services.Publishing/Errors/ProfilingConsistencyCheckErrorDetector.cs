...
namespace CalculateFunding.Services.Publishing.Errors
{
    public class ProfilingConsistencyCheckErrorDetector : PublishedProviderErrorDetector
    {
            ...
            ErrorCheck errorCheck)
        {
            ...
            //Story 97065 -bug 124641:below condition added for ILPREC 2223 becuse ILPREC 2223 work calcaluation ids only not consider distribution periods total
            if (distributionPeriodsTotal != totalExpectedFunding && publishedProviderVersion.FundingStreamId != "ILPREC" && (publishedProviderVersion.FundingPeriodId != "IL-2223" || publishedProviderVersion.FundingPeriodId != "IL-2324"))
            {
            ...
            ErrorCheck errorCheck)
        {
            foreach (DistributionPeriod distributionPeriod in fundingLine.DistributionPeriods ?? ArraySegment<DistributionPeriod>.Empty)
            {
                ...
                decimal profilePeriodsTotal = (distributionPeriod.ProfilePeriods?
                    ...
                    .GetValueOrDefault();
                //Story 97065 -bug 124641:below condition added for ILPREC 2223 becuse ILPREC 2223 work calcaluation ids only not consider distribution periods total
                if (totalExpectedFunding != profilePeriodsTotal && publishedProviderVersion.FundingStreamId != "ILPREC" && publishedProviderVersion.FundingPeriodId != "IL-2223")
                {
                    string fundingLineCode = fundingLine.FundingLineCode;
