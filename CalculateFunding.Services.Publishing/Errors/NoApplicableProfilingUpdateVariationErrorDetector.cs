        ...
        protected override Task<ErrorCheck> HasErrors(PublishedProvider publishedProvider, PublishedProvidersContext publishedProvidersContext)
        {
                ...
                !providerVariationContext.ApplicableVariations.AnyWithNullCheck(_ => _ == DISTRIBUTION_PROFILE_STRATEGY))
            {
                ...
                publishedProvider.Current.FundingLines = publishedProvider.Current.FundingLines?.Select(_ =>
                {
                    ...
                    //Ensuring a funding line with zero value overrides the existing profiling that is not changed through re-profiling
                    //when provider is changed from indicative to live
                    FundingLine priorFundingLine = providerVariationContext
                            .PriorState
                            .FundingLines?
                            .First(fl =>
                                fl.FundingLineCode == _.FundingLineCode);
                    if (providerVariationContext.PriorState.IsIndicative && !providerVariationContext.RefreshState.IsIndicative
                    && _.Value == 0 && priorFundingLine != null && priorFundingLine.Value != 0)
                    {
                    ...
                    //153563 :closed providers for PriorState and PreRefreshState and FundingLine is changed need to return the current data
                    //if (providerVariationContext.PriorState.Provider?.Status == "Closed" && providerVariationContext.PreRefreshState.Provider?.Status == "Closed"
                    //     && _.Value != currentFundingLine.Value)
                    //{
                    //    return _;
                    //}
                    ...
                    //below code for if ProfilePatternType is Calculation in the Profile config then consider current data
                    var calculationProfilePatternType = providerVariationContext.ProfilePatterns?.Where(x => x.Value?.ProfilePatternType == ProfilePatternType.Calculation);
                    if(calculationProfilePatternType != null && calculationProfilePatternType.Any())
                    {
                        return _;
                    }
