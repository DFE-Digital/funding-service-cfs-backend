...
namespace CalculateFunding.Services.Publishing
{
    public class VariationService : IVariationService
    {
        ...
        public async Task<ProviderVariationContext> PrepareVariedProviders(decimal? updatedTotalFunding,
            ...
            PublishedProviderVersion preRefreshProviderVersion, FundingConfiguration fundingConfiguration, bool applyReProfileVariationOnDemand)
        {
            ...
            ProviderVariationContext variationContext = await _detectProviderVariations.CreateRequiredVariationChanges(existingPublishedProvider,
                ...
                variances, fundingConfiguration, applyReProfileVariationOnDemand);

            if (variationContext.HasVariationChanges)
            {
                _applyProviderVariations.AddVariationContext(variationContext);
                //To differentiate the call to Reprofiling Service if it is from ReProfileOnDemand service or Refresh Service
                _applyProviderVariations.SetIsReProfileVariationAppliedOnDemand(applyReProfileVariationOnDemand);
            }
