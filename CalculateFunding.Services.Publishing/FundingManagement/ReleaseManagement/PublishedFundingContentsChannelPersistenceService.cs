...
namespace CalculateFunding.Services.Publishing.FundingManagement.ReleaseManagement
{
    public class PublishedFundingContentsChannelPersistenceService : IPublishedFundingContentsChannelPersistenceService
    {
        ...
        #region ProviderGroupingBackfillingService

        public async Task<bool> IsBlobExists(string blobName)
        {
            return await _blobClientPolicy.ExecuteAsync(() =>
                 _blobClient.BlobExistsAsync(blobName));
        }

        public async Task UploadBlobAsync(string blobName, string content, string specificationId)
        {
            await _blobClientPolicy.ExecuteAsync(() =>
                 UploadBlob(blobName, content, GetMetadata(specificationId)));

            _logger.Debug("Published funding contents updated to blob");
        }
        private static IDictionary<string, string> GetMetadata(string specificationId)
        {
            return new Dictionary<string, string>
            {
