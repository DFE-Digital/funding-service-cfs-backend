...
namespace CalculateFunding.Services.Publishing.FundingManagement.ReleaseManagement
{
    public class ReleaseProvidersToChannelsService : JobProcessingService, IReleaseProvidersToChannelsService
    {
            ...
            string correlationId)
        {
            ...
            if (releaseProvidersToChannelRequest.ProviderIds != null && releaseProvidersToChannelRequest.ProviderIds.Any())
            {
                string compressedProviderList = releaseProvidersToChannelRequest.ProviderIds.AsJson().CompressToText();
                releaseProvidersToChannelRequest.ProviderIds = new List<string> { compressedProviderList };
                _logger.Information("Compressed all the providers for the spec id '{specificationId}'", specificationId);
            }
        ...
        public override async Task Process(ServiceBusReceivedMessage message)
        {
            ...
            if (model.ProviderIds != null && model.ProviderIds.Any())
            {
                string decompressedProviderIds = model.ProviderIds.FirstOrDefault().DecompressFromText();

                _logger.Information("Decompressed all the providers for the spec id '{specificationId}'", specificationId);

                model.ProviderIds = decompressedProviderIds.AsJsonObject<List<string>>();
            }
