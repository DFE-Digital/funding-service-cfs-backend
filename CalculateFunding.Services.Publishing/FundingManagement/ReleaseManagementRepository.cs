...
namespace CalculateFunding.Services.Publishing.FundingManagement
{
    public class ReleaseManagementRepository : SqlRepository, IReleaseManagementRepository
    {
        ...
        public async Task UpdateFundingGroupVersionWithTotalFunding(string fundingId, decimal? totalFunding)
        {
            Guard.ArgumentNotNull(_transaction, nameof(_transaction));
            await QuerySingleSql<int>("UPDATE FundingGroupVersions SET TotalFunding=@totalFunding WHERE FundingId=@fundingId",
                new
                {
                    fundingId,
                    totalFunding
        ...
        public async Task<bool> IsExistsFundingGroupVersion(string fundingId)
        {
            int count =  await QuerySingleSql<int>("SELECT COUNT(1) FROM FundingGroupVersions WHERE FundingId=@fundingId",
                new
                {
                    fundingId
                });
        ...
        public async Task<IEnumerable<FundingGroupId>> QueryFundingId(
           int channelId,
           IEnumerable<string> fundingStreamIds,
           IEnumerable<string> fundingPeriodIds)
          
        {
            (string sql, object parameters) = _externalApiQueryBuilder.BuildFundingIdQuery(
                channelId,
                fundingStreamIds,
                fundingPeriodIds);

            return await QuerySql<FundingGroupId>(sql, parameters);
        }
        ...
        public async Task<IEnumerable<LatestFundingGroupVersionInProviderChannel>> GetLatestFundingGroupVersion(string specificationId)
        {
            return await QuerySql<LatestFundingGroupVersionInProviderChannel>($@"
                    SELECT FGV.FundingGroupVersionId, RPVC.ReleasedProviderVersionChannelId
	                ,FGV.ChannelVersion, FGV.ChannelId
                    , RP.ProviderId
                    , FG.OrganisationGroupTypeCode
                    , FG.OrganisationGroupIdentifierValue
                    , FG.OrganisationGroupName
                    ...
                    , FGV.TotalFunding AS GroupFundingTotal
                    , FGV.FundingId AS GroupFundingId
                    , FGV.StatusChangedDate
                    , FGV.EarliestPaymentAvailableDate
                    , FGV.ExternalPublicationDate
                    , FGV.MajorVersion
                FROM FundingGroupVersions FGV
                INNER JOIN 
                (SELECT FundingGroupId, MAX(MajorVersion) AS LatestVersion FROM FundingGroupVersions FGVAgg
        ...
        public async Task<IEnumerable<ReleasedProviderVersionChannel>> GetReleasedProviderVersionChannelUsingAmbientTransation(string specificationId, string providerId, int channelId)
        {
            Guard.ArgumentNotNull(_transaction, nameof(_transaction));

            return await QuerySql<ReleasedProviderVersionChannel>($@"
                SELECT  RPVC.* FROM ReleasedProviderVersionChannels RPVC
                INNER JOIN (SELECT ReleasedProviderVersionId, ReleasedProviderId, MAX(MajorVersion) As LatestVersion FROM ReleasedProviderVersions GROUP BY ReleasedProviderId, ReleasedProviderVersionId) RPV ON RPVC.ReleasedProviderVersionId = RPV.ReleasedProviderVersionId
                INNER JOIN ReleasedProviders RP ON RPV.ReleasedProviderId = RP.ReleasedProviderId
                WHERE RP.SpecificationId = @{nameof(specificationId)}
                AND RP.ProviderId = @{nameof(providerId)}
                AND RPVC.ChannelId = @{nameof(channelId)}",
                new
                {
                    specificationId,
        ...
        public async Task<IEnumerable<ReleasedProviderVersionChannelData>> GetReleasedProviderVersionChannel(string specificationId, int channelId)
        {
            return await QuerySql<ReleasedProviderVersionChannelData>($@"
                SELECT  RPVC.*, RPV.FundingId FROM ReleasedProviderVersionChannels RPVC
                INNER JOIN (SELECT FundingId, ReleasedProviderVersionId, ReleasedProviderId, MAX(MajorVersion) As LatestVersion FROM ReleasedProviderVersions GROUP BY ReleasedProviderId, ReleasedProviderVersionId, FundingId) RPV ON RPVC.ReleasedProviderVersionId = RPV.ReleasedProviderVersionId
                INNER JOIN ReleasedProviders RP ON RPV.ReleasedProviderId = RP.ReleasedProviderId
                WHERE RP.SpecificationId = @{nameof(specificationId)}
                AND RPVC.ChannelId = @{nameof(channelId)}",
                new
