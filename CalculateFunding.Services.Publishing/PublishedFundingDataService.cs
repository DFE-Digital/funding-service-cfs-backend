...
namespace CalculateFunding.Services.Publishing
{
    public class PublishedFundingDataService : IPublishedFundingDataService
    {
        ...
        public async Task<IEnumerable<PublishedProvider>> GetPublishedProvidersForApproval(string specificationId, string[] publishedProviderIds = null)
        {
            ...
            {
                var startDate = DateTime.Now.ToLocalTime();
                _logger.Information("Execuation started at {StartTime}" , startDate);                
                ...
                _logger.Information("Execuation Ended at {EndTime}", endTime);
        ...
        public async Task<IEnumerable<PublishedProvider>> GetReleasedPublishedProviders(string fundingStreamId, string specificationId)
        {
            Guard.IsNullOrWhiteSpace(fundingStreamId, nameof(fundingStreamId));
            Guard.IsNullOrWhiteSpace(specificationId, nameof(specificationId));

            IEnumerable<KeyValuePair<string, string>> publishedProviderIds = await _publishedFundingRepositoryPolicy.ExecuteAsync(
                                () => _publishedFundingRepository.GetPublishedReleasedProviderIds(fundingStreamId, specificationId));
