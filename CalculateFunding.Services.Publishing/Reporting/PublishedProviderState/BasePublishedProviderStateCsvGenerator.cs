...
namespace CalculateFunding.Services.Publishing.Reporting.PublishedProviderState
{
    public abstract class BasePublishedProviderStateCsvGenerator : JobProcessingService, IPublishedProviderEstateCsvGenerator, IPublishedProviderStateSummaryCsvGenerator, IHealthChecker
    {
        ...
        protected abstract string GetCsvFileName(ServiceBusReceivedMessage message);

        protected abstract IDictionary<string, string> GetMetadata(ServiceBusReceivedMessage message);

        protected abstract Task<bool> GenerateCsv(ServiceBusReceivedMessage message, string temporaryFilePath, IPublishedProviderStateSummaryCsvTransform publishedProviderCsvTransform);
        ...
        private string GetCsvFilePath(string rootPath, ServiceBusReceivedMessage message)
        {
            return Path.Combine(rootPath, GetCsvFileName(message));
        }

        protected abstract string GetContentDisposition(ServiceBusReceivedMessage message);
    }
}
