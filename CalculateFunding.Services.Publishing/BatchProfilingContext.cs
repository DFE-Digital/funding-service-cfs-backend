...
namespace CalculateFunding.Services.Publishing
{
    public class BatchProfilingContext : PagedContext<BatchProfilingRequestModel>, IBatchProfilingContext
    {
        ...
        public void AddProviderProfilingRequestData(PublishedProvider publishedProvider,
            ...
            bool isNewProvider,
            bool enableProfilingBasedOnOpenDate)
        {
            ...
            {
                //Provider Type, SubType, OpenDate & OpenReason is required in the request for the new providers that are not released yet
                //for the funding stream (PP) that enabled profiling based on open date
                ...
                ProfilingRequests.Add(new ProviderProfilingRequestData
                {
                    ...
                    ProviderType = isNewProvider || (enableProfilingBasedOnOpenDate && releasedProviderVersion == null) ? provider.ProviderType : null,
                    ProviderSubType = isNewProvider || (enableProfilingBasedOnOpenDate && releasedProviderVersion == null) ? provider.ProviderSubType : null,
                    DateOpened = (isNewProvider || releasedProviderVersion == null) && enableProfilingBasedOnOpenDate ? provider.DateOpened : null,
                    ReasonEstablishmentOpened = (isNewProvider || releasedProviderVersion == null) && enableProfilingBasedOnOpenDate ? provider.ReasonEstablishmentOpened : null,
            ...
            if (response.ProfilePatternType == ProfilePatternType.Calculation)
            {
                foreach (PublishedProviderVersion publishedProvider in batch.PublishedProviders)
                {
                    ...
                    DistributionPeriod[] publishedProviderDistributionPeriods = response.DistributionPeriods.Select(_ => new DistributionPeriod
                    {
                        ...
                        Value = response.DeliveryProfilePeriods.Where(pp =>
                                pp.DistributionPeriod == _.DistributionPeriodCode).Sum(pp => Convert.ToDecimal(generatedPublishedProviderData[publishedProvider.ProviderId].Calculations?.FirstOrDefault(_ => _.TemplateCalculationId == pp.CalculationId)?.Value ?? 0)),
