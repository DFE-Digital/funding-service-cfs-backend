...
namespace CalculateFunding.Services.Publishing
{
    public class RefreshService : JobProcessingService, IRefreshService
    {
            ...
            Reference fundingPeriod)
        {
            ...
            try
            {
                await ProfileProviders(publishedProviders, _refreshStateService.NewProviders, generatedPublishedProviderData, fundingConfiguration.EnableProfilingBasedOnOpenDate);
            }
            catch (Exception ex)
            {
            ...
            if (fundingConfiguration?.NonRestrictedProviderMetaData != null && fundingConfiguration.NonRestrictedProviderMetaData.Any())
            {
                IEnumerable<string> limitedVariances = fundingConfiguration.NonRestrictedProviderMetaData.Distinct();
                _publishedProviderDataPopulator.SetLimitedVariances(limitedVariances);
            }
            ...
            foreach (KeyValuePair<string, PublishedProvider> publishedProvider in publishedProvidersReadonlyDictionary)
            {
                ...
                if (publishedProviderUpdated && existingPublishedProviders.AnyWithNullCheck() && scopedProviders.ContainsKey(providerId))
                {
                    ProviderVariationContext context = await _variationService.PrepareVariedProviders(generatedProviderResult.TotalFunding ?? 0,
                        ...
                        scopedProviders[providerId],
                        fundingConfiguration?.Variations ?? fundingConfiguration?.ReleaseManagementVariations,
                        variationPointers,
                        fundingStream.Id,
                        specification.ProviderVersionId,
                        ...
                        fundingStream.Id,
                        fundingPeriod.Id,
                        preRefreshProviderVersion,fundingConfiguration, false);
            ...
            if (_refreshStateService.Count > 0)
            {
                ...
                int jobsCount = await _jobManagement.GetJobsCountByJobDefinitionIdAndStatus(specification.Id, JobConstants.DefinitionNames.RefreshFundingJob, nameof(RunningStatus.Completed), nameof(CompletionStatus.Succeeded));
        ...
        bool enableProfilingBasedOnOpenDate)
        {
            ...
            foreach (KeyValuePair<string, PublishedProvider> publishedProvider in publishedProviders)
            {
                ...
                batchProfilingContext.AddProviderProfilingRequestData(publishedProvider.Value,
                    ...
                    isNewProvider,
                    enableProfilingBasedOnOpenDate);
            }
