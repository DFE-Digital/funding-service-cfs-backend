...
namespace CalculateFunding.Services.Specs.Interfaces
{
    public interface ISpecificationsRepository
    {
        ...
        Task<HttpStatusCode> UpdateSpecification(Specification specification, bool allowCommit = false);
        ...
        Task<IEnumerable<Specification>> GetSpecificationsBySQLQuery(Expression<Func<EntityModel.Specification, bool>> query);

        bool IsConnectedToSQL();

        Task<IEnumerable<SpecificationProfileVariationPointerModel>> GetProfileVariationPointers(string specificationId);

        Task<IEnumerable<Specification>> GetSpecificationsWithProviderVersionUpdatesAsUseLatest(Expression<Func<EntityModel.SpecificationVersion, bool>> query);
    }
}