...
namespace CalculateFunding.Services.Specs
{
    public class SpecificationsService : JobProcessingService, ISpecificationsService, IHealthChecker
    {
        ...
        public async Task<IActionResult> GetSpecificationSummariesByIds(string[] specificationIds)
        {
            ...
            if (_specificationsRepository.IsConnectedToSQL())
            {
                specifications = await _specificationsRepository.GetSpecificationsBySQLQuery(c => specificationIds.Contains(c.SpecificationId));
            }
            else
            {
                specifications =
    ...
    string fundingPeriodId, string fundingStreamId)
        ...
        public async Task<IActionResult> GetSpecificationsSelectedForFunding()
        {
            ...
            {
                specifications =
                   (await _specificationsRepository.GetSpecificationsBySQLQuery(c => c.IsSelectedForFunding))
        ...
        public async Task<IActionResult> GetSpecificationsSelectedForFundingByPeriod(string fundingPeriodId)
        {
            ...
            {
                specifications =
                   (await _specificationsRepository.GetSpecificationsBySQLQuery(c => c.IsSelectedForFunding && c.FundingPeriod.FundingPeriodCode == fundingPeriodId))
                   .Select(s => _mapper.Map<SpecificationSummary>(s));
            }
            else
            {
        ...
        public async Task<IActionResult> GetFundingStreamsSelectedForFundingBySpecification(string specificationId)
        {
            ...
            {
                fundingStreams = (
                   await _specificationsRepository.GetSpecificationsBySQLQuery(c => c.IsSelectedForFunding && c.SpecificationId == specificationId)
        ...
        public async Task<IActionResult> GetSpecificationByName(string specificationName)
        {
            ...
            {
                specifications =
                   await _specificationsRepository.GetSpecificationsBySQLQuery(c => c.SpecificationName == specificationName.ToLower());
            ...
            foreach (string fundingStreamId in specification.Current.FundingStreams.Select(m => m.Id))
            {
                ...
                if (specification.Current.FundingStreams.FirstOrDefault()?.Id == fundingStreamId)
                {
                    ...
                    if (fundingConfiguration.ProviderSource == ProviderSource.FDZ)
                    {
                        ...
                        {
                            ApiResponse<CurrentProviderVersionMetadata> providerMetadataResponse;
                            //Consuming provider data as ADF for ALL and ASF Specs - US 162881 and Task 195544       
                            if (AdultStream.IsExists(fundingStreamId))
                            {
                               string adultFundingStreamId = AdultStream.GetParent();
                                providerMetadataResponse = await _providersApiClientPolicy.ExecuteAsync(
                               () => _providersApiClient.GetCurrentProviderMetadataForFundingStream(adultFundingStreamId));
                            }
                            else
                            {
                                providerMetadataResponse = await _providersApiClientPolicy.ExecuteAsync(
            ...
            string correlationId)
        {
            ...
            if (editModel.CoreProviderVersionUpdates == CoreProviderVersionUpdates.UseLatest)
            {
                //Consuming provider data as ADF for ALL and ASF Specs - US 162881 and Task 195544        
                if (AdultStream.IsExists(fundingStreamId))
                {
        ...
        public async Task<IActionResult> ReIndex()
        {
            ...
            try
            {
                if (_specificationsRepository.IsConnectedToSQL())
                {
                    IEnumerable<Specification> specData = await _specificationsRepository.GetSpecifications();
                    specifications = specData.Select(_ => new SpecificationSearchModel()
                    {
                        DataDefinitionRelationshipIds = _.Current.DataDefinitionRelationshipIds.ToArray(),
                        Name = _.Name,
                        FundingStreams = _.Current.FundingStreams,
                        FundingPeriod = _.Current.FundingPeriod,
                        PublishStatus = _.Current.PublishStatus.ToString(),
                        Id = _.Id,
                        Description = _.Current.Description,
                        IsSelectedForFunding = _.IsSelectedForFunding,
                        UpdatedAt = _.Current.UpdatedAt
                    });
                }
                else
                {
...
WHERE   s.documentType = @DocumentType",
        ...
        private async Task<IActionResult> UpdateSpecification(Specification specification, string specificationId)
        {
            try
            {
                HttpStatusCode statusCode = await _specificationsRepository.UpdateSpecification(specification, true);
            ...
            catch (Exception ex)
            {
                ...
                await TaskHelper.WhenAllAndThrow(
                    _specificationsRepository.UpdateSpecification(specification, true),
        ...
        public async Task<IActionResult> GetProfileVariationPointers(string specificationId)
        {
            ...
            if (_specificationsRepository.IsConnectedToSQL())
            {
                ...
                return new OkObjectResult(await _specificationsRepository.GetProfileVariationPointers(specificationId));
            }
            else
            {
                Specification specification = await _specificationsRepository.GetSpecificationById(specificationId);
            ...
            string fundingStreamId)
        {
            ...
            {
                specificationsByQuery =
               await _specificationsRepository.GetSpecificationsBySQLQuery(c =>
                   c.IsSelectedForFunding &&
                   c.FundingStream.FundingStreamCode == fundingStreamId);
            }
            else
            {
                specificationsByQuery =
        ...
        public async Task DeleteSpecification(ServiceBusReceivedMessage message)
        {
        ...
        public async Task<IActionResult> GetSpecificationsWithProviderVersionUpdatesAsUseLatest()
        {
            ...
            if (_specificationsRepository.IsConnectedToSQL())
            {
                    ...
                    specifications = (
                 await _specificationsRepository.GetSpecificationsWithProviderVersionUpdatesAsUseLatest(_ => _.CoreProviderVersionUpdates == CoreProviderVersionUpdates.UseLatest.ToString() && !_.IsDeleted && _.IsLatest))
                 .Select(s => _mapper.Map<SpecificationSummary>(s));
