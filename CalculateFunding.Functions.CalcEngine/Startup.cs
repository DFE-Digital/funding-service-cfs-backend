        ...
        private static IServiceProvider Register(IServiceCollection builder, IConfigurationRoot config)
        {
                ...
                builder.AddSingleton<IProviderSourceDatasetsRepository, CosmosProviderSourceDatasetsRepository>((ctx) =>
            {
                ...
                CosmosRepository calcsCosmosRepository = (Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == "Development")
                    ? new CosmosRepository(providerSourceDatasetsCosmosSettings, new CosmosClientOptions()
                    {
                ...
                return new CosmosProviderSourceDatasetsRepository(calcsCosmosRepository, calculatorResiliencePolicies);
            });
            }
            ...
            else
            {
                ...
                builder.AddSingleton<IProviderResultsRepository, CosmosProviderResultsRepository>((ctx) =>
                {
                    ...
                    CosmosRepository calcsCosmosRepostory = (Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == "Development")
                        ? new CosmosRepository(calcResultsDbSettings, new CosmosClientOptions()
                        {
                            ConnectionMode = ConnectionMode.Gateway,
                            RequestTimeout = new TimeSpan(0, 0, 15),
                            AllowBulkExecution = true,
                    ...
                    return new CosmosProviderResultsRepository(
            ...
            builder.AddScoped<ICalculationEngineServiceValidator, CalculationEngineServiceValidator>();
            builder.AddSingleton<ISpecificationAssemblyProvider, SpecificationAssemblyProvider>();
            builder.AddSingleton<CalculateFunding.Services.Core.Interfaces.AzureStorage.IBlobClient>(ctx =>
            {
                AzureStorageSettings options = new AzureStorageSettings();

                config.Bind("AzureStorageSettings", options);

                options.ContainerName = "source";

                return new CalculateFunding.Services.Core.AzureStorage.BlobClient(options);
            });
