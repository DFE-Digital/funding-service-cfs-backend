...
namespace CalculateFunding.Runners.CalcEngine
{
    public abstract class ServiceBusQueueWorker<TMessage> : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            var connectionString = Configuration.GetValue<string>("ServiceBusSettings:ConnectionString");
            var managementClient = new ServiceBusAdministrationClient(connectionString);

            var runtimeInfo = await managementClient.GetQueueRuntimePropertiesAsync(queueName);
            long messageCount = runtimeInfo.Value.ActiveMessageCount;
            ...
            while (!stoppingToken.IsCancellationRequested && messageCount > 0)
            {
                ...
                runtimeInfo = await managementClient.GetQueueRuntimePropertiesAsync(queueName);
                messageCount = runtimeInfo.Value.ActiveMessageCount;
            }
        ...
        private async Task HandleMessageAsync(ProcessSessionMessageEventArgs processMessageEventArgs)
        {
            try
            {
                string rawMessageBody = processMessageEventArgs.Message.Body.ToString();
        ...
        protected abstract Task ProcessMessage(TMessage order, string messageId, IReadOnlyDictionary<string, object> applicationProperties, CancellationToken cancellationToken);
    }
}