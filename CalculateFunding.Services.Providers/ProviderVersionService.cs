...
namespace CalculateFunding.Services.Providers
{
    public class ProviderVersionService : IProviderVersionService, IHealthChecker
    {
        ...
        public async Task<IActionResult> GetAllProviders(string providerVersionId)
        {
            ...
            SdkBlobClient blob = await _blobClient.GetBlobReferenceFromServerAsync(providerVersionId.ToLowerInvariant() + ".json");

            bool blobExists = await _blobRepositoryPolicy.ExecuteAsync(() => _blobClient.BlobExistsAsync(blobName.ToLowerInvariant()));
        ...
        private async Task UploadProviderVersionBlob(string providerVersionId, ProviderVersion providerVersion)
        {
            ...
            // Use the IBlobClient abstraction to upload so unit tests can substitute UploadAsync without
            // needing a concrete Azure SDK BlobClient implementation.
            await _blobRepositoryPolicy.ExecuteAsync(() => _blobClient.UploadAsync(blob, stream));
        ...
        public async Task<IActionResult> GetProviderVersionMetadata(string providerVersionId)
        {
            ...
            if (result == null)
            {
                ProviderVersionMetadata providerVersionMetadata =
                    await _providerVersionMetadataRepositoryPolicy.ExecuteAsync(async () => await _providerVersionMetadataRepository.GetProviderVersionMetadata(providerVersionId));
