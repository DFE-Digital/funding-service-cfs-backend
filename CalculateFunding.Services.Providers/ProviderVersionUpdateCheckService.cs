    ...
    public class ProviderVersionUpdateCheckService : IProviderVersionUpdateCheckService
    {
        ...
        public async Task CheckProviderVersionUpdate()
        {
            ...
            foreach (FundingStream fundingStream in fundingStreams)
            {
                ...
                string currentfundingStreamId = fundingStream.Id;
                ...
                //Consuming provider data as ADF for ALL and ASF Specs - US 162881  for fixing autotrack update      
                if (AdultStream.IsExists(fundingStreamId))
                {
                  ...
                  .Select(_ => new ProviderSnapShotByFundingPeriod() { FundingPeriodName = _.FundingPeriodName, ProviderSnapshotId = _.ProviderSnapshotId }).ToList();
                   ...
                   latestProviderSnapshot = providerSnapshots.FirstOrDefault(x => x.FundingStreamCode == fundingStreamId);
                }
                else
                {
                    latestproviderSnapShotByFundingPeriod = SnapshotIdWithFundingPeriod.Where(_ => _.FundingStreamCode == fundingStreamId)
                ...
                fundingStreamId = currentfundingStreamId;
        ...
        private async Task<ProviderVersionMetadata> GetProviderVersion(string providerVersionId)
        {
            ProviderVersionMetadata providerVersion =
                await _providerVersionMetadataPolicy.ExecuteAsync(async () =>
                   await _providerVersionMetadata.GetProviderVersionMetadata(providerVersionId));
