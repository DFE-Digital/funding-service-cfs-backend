...
namespace CalculateFunding.Services.Calcs.Analysis.ObsoleteItems
{
    public abstract class ObsoleteItemsReferenceCleanUp : IObsoleteReferenceCleanUp
    {
        ...
        protected async Task<IEnumerable<DocumentEntity<ObsoleteItem>>> GetObsoleteDocumentsForCalculation(string calculationId)
            => await _calculations.GetObsoleteItemDocumentsForCalculation(calculationId, _obsoleteItemType);
        ...
        private async Task ReadObsoleteItemsAndProcessForCalculation(Calculation calculation)
        {
            ...
            IEnumerable<DocumentEntity<ObsoleteItem>> obsoleteItemsInCalculation = await GetObsoleteDocumentsForCalculation(calculationId);
