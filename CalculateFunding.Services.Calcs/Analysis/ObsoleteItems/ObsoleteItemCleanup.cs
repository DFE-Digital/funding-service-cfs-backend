...
namespace CalculateFunding.Services.Calcs.Analysis.ObsoleteItems
{
    public class ObsoleteItemCleanup : IObsoleteItemCleanup
    {
        ...
        public async Task ProcessCalculation(Calculation calculation)
        {
            ...
            //EfCore DB Context doesnot support concurrent/multi-thread operations
            foreach (var itemReferenceCleanUpStep in _obsoleteItemReferenceCleanUpSteps)
            {
                await itemReferenceCleanUpStep.ProcessCalculation(calculation);
            }
        }
    }
}