...
namespace CalculateFunding.Services.Calcs
{
    public class PreviewService : IPreviewService, IHealthChecker
    {
        ...
        public async Task<IActionResult> Compile(PreviewRequest previewRequest)
        {
            ...
            IEnumerable<Calculation> calculations = _calculationsRepository.GetCalculationsBySpecificationId(previewRequest.SpecificationId).Result;
            ...
            await TaskHelper.WhenAllAndThrow(buildProjectTask, compilerOptionsTask);
            ...
            List<Calculation> allSpecCalculations = new List<Calculation>(calculations);
            ...
            PreviewRequest previewRequest)
        {
            ...
            IEnumerable<string> aggregateParameters = Enumerable.Empty<string>();
            List<CalcEngineModels.CalculationAggregationData> calculationAggregationData = new List<CalcEngineModels.CalculationAggregationData>();
            ...
            if (compilerOutput.Success)
            {
                ...
                else
                {
                    if (previewRequest != null)
                    {
                        ...
                        {
                            aggregateParameters = SourceCodeHelpers.GetCalculationAggregateFunctionParameters(previewRequest.SourceCode);
                ...
                //Calc Aggregation mapping with Calculation to get id for later querying.
                foreach(string aggregateParameter in aggregateParameters)
                {
                    Calculation aggregableCalculation = calculations.Where(_ => aggregateParameter.Equals($"{_typeIdentifierGenerator.GenerateIdentifier(_.Namespace)}.{_typeIdentifierGenerator.GenerateIdentifier(_.Name)}")).FirstOrDefault();
                    if(aggregableCalculation != null)
                    {
                        calculationAggregationData.Add(new CalcEngineModels.CalculationAggregationData()
                        {
                            AggregateFunctionName = aggregateParameter,
                            CalculationId = aggregableCalculation.Id
                        });
                    }
                }
            }
            ...
            {
                //Remove the calculation aggregation cache
                string calculationAggregatesCacheKeyPrefix = $"{CacheKeys.CalculationAggregations}{buildProject.SpecificationId}";
...
Calculation Name: {{calculationName}}";
        ...
        private async Task<Build> CheckDatasetValidAggregations(PreviewRequest previewRequest)
        {
            ...
            IEnumerable<string> datasetAggregationFields = datasetSchemaRelationshipModels?.SelectMany(m => m.Fields?.Where(f => f.IsAggregable && f.IsNumeric).Select(f => f.FullyQualifiedSourceName));
