...
namespace CalculateFunding.Services.Calcs
{
    public class BuildProjectsService : JobProcessingService, IBuildProjectsService, IHealthChecker
    {
            ...
            IFDSApiClient fdsAPIClient) : base(jobManagement, logger)
        {
            ...
            _fdsApiClientPolicy = resiliencePolicies.FDSApiClientPolicy;
        ...
        public override async Task Process(ServiceBusReceivedMessage message)
        {
            ...
            string specificationSummaryCachekey = message.ApplicationProperties.ContainsKey("specification-summary-cache-key") ?
                message.ApplicationProperties["specification-summary-cache-key"].ToString() :
            ...
            string providerCacheKey = message.ApplicationProperties.ContainsKey("provider-cache-key") ?
                message.ApplicationProperties["provider-cache-key"].ToString() :
        ...
        public async Task UpdateBuildProjectRelationships(ServiceBusReceivedMessage message)
        {
            Guard.ArgumentNotNull(message, nameof(message));
        ...
        private async Task<IEnumerable<Job>> CreateGenerateAllocationJobs(IEnumerable<IDictionary<string, string>> jobProperties, string specificationId)
        {
            ...
            foreach (Models.Calcs.Calculation calculation in calculations)
            {
                ...
                {
                    Models.Calcs.Calculation referencedCalculation = calculations.FirstOrDefault(m => aggregateParameter.Trim().Equals(_typeIdentifierGenerator.GenerateIdentifier(m.Namespace)+"."+ m.Current.SourceCodeName));

                    if (referencedCalculation != null)
                    {
                        calculationsToAggregate.Add(aggregateParameter + ":" + referencedCalculation.Id);
                    }
                }
            }
        ...
        private async Task<BuildProject> GenerateBuildProject(string specificationId, IEnumerable<(Reference, Reference)> fundingStreamAndPeriods, IDictionary<string, string> templateIds)
        {
            ...
            if (!datasetsApiClientResponse.StatusCode.IsSuccess())
            {
                ...
                _logger.Error($"No current dataset relationships found for specificationId '{specificationId}'.");
            ...
            if (!datasetRelationshipModels.IsNullOrEmpty())
            {
                ...
                {
                    if(datasetRelationshipModel.Definition == null) continue;
                    Task task = Task.Run(async () =>
                    {
                        string definitionId = datasetRelationshipModel.Definition.Id;
                        if (datasetRelationshipModel.RelationshipType == Models.Datasets.DatasetRelationshipType.FDS)
                        {
                            ...
                            {
                                string message = $"FDS API Failure: No FDS DatasetDefinition found for definition id - '{definitionId}'.";
                ...
                foreach (DatasetSpecificationRelationshipViewModel datasetRelationshipModel in datasetRelationshipModels)
                {
                    ...
                    DatasetDefinition datasetDefinition = datasetDefinitions.FirstOrDefault(m => m.Id == datasetRelationshipModel.Definition?.Id);
                    ...
                    DatasetRelationshipSummary datasetRelationshipSummary = new DatasetRelationshipSummary
                    {
                        ...
                        DataGranularity  = (datasetDefinition == null || datasetDefinition.DataGranularity.IsNullOrEmpty())
                                ? DataGranularity.SingleRowPerProvider
                                : (DataGranularity)Enum.Parse(typeof(DataGranularity), datasetDefinition.DataGranularity),
        ...
        public async Task<IActionResult> GetCompiledBuildProjectBySpecificationId(string specificationId)
        {
            if (string.IsNullOrWhiteSpace(specificationId))
            {
                _logger.Error("No specification Id was provided to GetBuildProjectBySpecificationId");
            ...
            {
                string message = "Build project is null or not found";
                _logger.Error(message);
