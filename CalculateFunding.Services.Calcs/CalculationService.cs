...
namespace CalculateFunding.Services.Calcs
{
    public class CalculationService : ProcessingService, ICalculationService, IHealthChecker
    {
        ...
        public async Task DeleteCalculations(ServiceBusReceivedMessage message)
        {
...
End Select");
        ...
        {
            IEnumerable<Calculation> calculations = _calculationRepositoryPolicy.ExecuteAsync(() => _calculationsRepository.GetCalculationsBySpecificationId(specificationSummary.Id)).Result;
            Task<BuildProject> buildProjectRequest = _buildProjectsService.GetBuildProjectForSpecificationId(specificationSummary.Id);

            await TaskHelper.WhenAllAndThrow(buildProjectRequest);
        ...
        private async Task<HttpStatusCode> UpdateCalculation(Calculation calculation, CalculationVersion calculationVersion, CalculationVersion previousVersion)
        {
            ...
            if (!_calculationsRepository.IsConnectedToSQL())
            {
            ...
            {
                return await _calculationVersionsRepositoryPolicy.ExecuteAsync(() => _calculationVersionRepository.SaveVersion(calculationVersion));
            }
