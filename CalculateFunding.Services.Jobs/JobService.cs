...
namespace CalculateFunding.Services.Jobs
{
    public class JobService : IJobService, IHealthChecker
    {
        ...
        public async Task<IActionResult> GetLatestJobByTriggerEntityId(string specificationId, string entityId)
        {
            ...
            JobSummary jobSummary = _mapper.Map<JobSummary>(await GetJobFromLatestCache(() 
        ...
        public async Task<IActionResult> GetJobsCountByJobDefinitionIdAndStatus(string specificationId, string jobDefinitionId, string runningStatus, string completionStatus)
        {
            Guard.ArgumentNotNull(specificationId, nameof(specificationId));
            ...
            int jobsCount =  await _jobRepository.GetJobsCountByJobDefinitionIdAndStatus(specificationId, jobDefinitionId, runningStatus, completionStatus);

            return new OkObjectResult(jobsCount);
        }
        ...
        private async Task<Job> GetJobFromLatestCache(Func<Task<Job>> getJob, string cacheKey)
        {         
            JobCacheItem jobCacheItem = await _cacheProviderPolicy.ExecuteAsync(() => _cacheProvider.GetAsync<JobCacheItem>(cacheKey));
            Job job = jobCacheItem?.Job;

            Job latestJobs = await getJob();
            if (job != null && job?.RunningStatus != latestJobs?.RunningStatus)
            {
                await _cacheProviderPolicy.ExecuteAsync(() => _cacheProvider.RemoveAsync<JobCacheItem>(cacheKey));
                jobCacheItem = null;
                job = null;               
            }
