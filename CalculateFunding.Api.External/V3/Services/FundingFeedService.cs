    ...
    public class FundingFeedService : IFundingFeedService
    {
        ...
        private async Task CreateAtomFeed(SearchFeedResult<PublishedFundingIndex> searchFeed, HttpRequest request, HttpResponse response)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                //
                //User Story 131043 : below code added because we are stoping to create blobs for V3 api 
                var contentDocuments = contents.Where(x => x.Value != null);

                bool isLastBatch = i == list.Count - 1;

                await AddAtomEntryAsync(request, response.BodyWriter, fundingTrimmedRequestPath, contentDocuments, isLastBatch);
            }
