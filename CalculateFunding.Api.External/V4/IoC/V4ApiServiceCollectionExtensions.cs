    ...
    public static class V4ApiServiceCollectionExtensions
    {
        public static IServiceCollection AddExternalApiV4Services(this IServiceCollection builder, IConfiguration configuration)
        {
            ...
            builder.AddSingleton<IProviderFundingVersionService>((ctx) =>
            {
                ...
                IPublishingResiliencePolicies publishingResilience = ctx.GetService<IPublishingResiliencePolicies>();
                IExternalApiFeedWriter externalApiFeedWriter = ctx.GetService<IExternalApiFeedWriter>();               
                IExternalEngineOptions externalEngineOptions = ctx.GetService<IExternalEngineOptions>();

                return new ProviderFundingVersionService(blobClient, releaseManagementRepository, channelUrlToChannelResolver, blobDocumentPathGenerator, logger, publishingResiliencePolicies, fileSystemCache, settings, externalApiFeedWriter, publishingResilience, externalEngineOptions);
            });
