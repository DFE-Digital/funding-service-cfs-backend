...
namespace CalculateFunding.Services.Profiling.Models
{
    public class ProfileBatchRequest : ProfileRequestBase
    {
        ...
        public string GetFundingValueKey(decimal fundingValue)
            => $"{FundingPeriodId}-{FundingStreamId}-{ProfilePatternKey ?? "?"}-{ProviderType ?? "?"}-{ProviderSubType ?? "?"}-{(DateOpened.HasValue ? DateOpened.ToString() : "?")}-{ReasonEstablishmentOpened ?? "?"}-{FundingLineCode}-{fundingValue:N4}";
    }
}