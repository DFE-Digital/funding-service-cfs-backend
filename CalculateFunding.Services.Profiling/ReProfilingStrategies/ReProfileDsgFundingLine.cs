...
namespace CalculateFunding.Services.Profiling.ReProfilingStrategies
{
    public class ReProfileDsgFundingLine : ReProfilingStrategy, IReProfilingStrategy
    {
        ...
        public ReProfileDsgFundingLine(ILogger logger)
        {
        ...
        private decimal AdjustForUnderOrOverPayment(ReProfileContext context)
        {
            ...
            _logger.Information($"previousFundingLineValuePaid : {previousFundingLineValuePaid} and latestFundingLineValuePaid : {latestFundingLineValuePaid} and latestFundingLineValue {latestFundingLineValue} for latest period amount : {latestPeriodAmount}");

            AdjustPeriodsForFundingAlreadyReleased(variationPointerIndex,
                orderedExistingProfilePeriods,
                orderedRefreshProfilePeriods);
            _logger.Information($"Funding Change: {fundingChange}");
            if (fundingChange < 0)
            {
                _logger.Information($"Funding Change is less than 0");
                    ...
                    out decimal remainingOverPayment))
                {
                   ...
                   _logger.Information($"CarryOver Amount when funding change is less than 0: {carryOverAmount}");
                }
            }
            else if (fundingChange > 0)
            {
                _logger.Information($"Funding Change is greater than 0");
            ...
            {
                _logger.Information($"Funding Change is equal to 0");
            ...
            _logger.Information($"CarryOver Amount: {carryOverAmount}");
