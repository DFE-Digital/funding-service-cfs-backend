...
namespace CalculateFunding.Services.Profiling.ReProfilingStrategies
{
    public class ReProfileFlatDistributionForRemainingPeriods : FlatDistributionReProfilingStrategy, IReProfilingStrategy
    {
        ...
        public ReProfileStrategyResult ReProfile(ReProfileContext context)
        {
            ...
            int variationPointerIndex = (reProfileRequest.IsReProfileOnDemandTriggered && reProfileRequest.DateOpened.HasValue)
                ? GetVariationPointerIndexBasedOnDateOpened(orderedRefreshProfilePeriods, context)
                : GetVariationPointerIndex(orderedRefreshProfilePeriods, orderedExistingProfilePeriods, context);
