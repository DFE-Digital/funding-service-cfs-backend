...
namespace CalculateFunding.Services.Profiling.Services
{
    public class CalculateProfileService : ICalculateProfileService, IHealthChecker
    {
        ...
        {
            if(profileRequest.DateOpened.HasValue && !string.IsNullOrWhiteSpace(profileRequest.ProviderType) && !string.IsNullOrWhiteSpace(profileRequest.ReasonEstablishmentOpened)) {
                return await GetProfilePatternByOpenDateAndProviderType(profileRequest);
            }
            else if (!string.IsNullOrWhiteSpace(profileRequest.ProviderType) && !string.IsNullOrWhiteSpace(profileRequest.ProviderSubType)) {
                return await GetProfilePatternByProviderTypes(profileRequest);
            }
            else {
                return await GetProfilePatternByIdOrDefault(profileRequest);
            }
        }
        ...
        private string GetProfilePatternCacheKeyByOpenDateAndProviderType(ProfileRequestBase profileRequest)
        {
            string providerTypeIdComponent = string.IsNullOrWhiteSpace(profileRequest.ProviderType) ? null : $"-{profileRequest.ProviderType}";
            string providerOpenDateComponent = !profileRequest.DateOpened.HasValue ? null : $"-{profileRequest.DateOpened.Value}";
            string providerReasonOpenedComponent = string.IsNullOrWhiteSpace(profileRequest.ReasonEstablishmentOpened) ? null : $"-{profileRequest.ReasonEstablishmentOpened}";

            return $"{profileRequest.FundingPeriodId}-{profileRequest.FundingStreamId}-{profileRequest.FundingLineCode}{providerTypeIdComponent}{providerOpenDateComponent}{providerReasonOpenedComponent}";
        }
        ...
        private async Task<FundingStreamPeriodProfilePattern> GetProfilePatternByOpenDateAndProviderType(ProfileRequestBase profileRequest)
        {
            FundingStreamPeriodProfilePattern profilePattern = null;

            string profilePatternCacheKey = GetProfilePatternCacheKeyByOpenDateAndProviderType(profileRequest);
            ...
            if (profilePattern == null)
            {
                profilePattern = await _profilePatternRepositoryResilience.ExecuteAsync(() =>
                    _profilePatternRepository.GetProfilePattern(profileRequest.FundingPeriodId,
                        ...
                        profileRequest.FundingLineCode,
                        profileRequest.ProviderType,
                        profileRequest.DateOpened,
                        profileRequest.ReasonEstablishmentOpened));
