...
namespace CalculateFunding.Services.Datasets
{
    public class ProcessDatasetService : JobProcessingService, IProcessDatasetService, IHealthChecker
    {
        ...
        public ProcessDatasetService(
            ...
            IProviderSourceDatasetBulkRepository bulkProviderSourceDatasetRepository,
            IFDSApiClient fdSApiClient) : base(jobManagement, logger)
        {
        ...
        public override async Task Process(ServiceBusReceivedMessage message)
        {
            ...
            string parentJobId = message.ApplicationProperties.ContainsKey("parentJobId") ? message.ApplicationProperties["parentJobId"].ToString() : null;
            bool isScopedJob = false;

            if (message.ApplicationProperties.ContainsKey("isScopedJob") && message.ApplicationProperties["isScopedJob"] != null)
            {
                isScopedJob = bool.Parse(message.ApplicationProperties["isScopedJob"].ToString());
            }

            // if this is a scoped job then we don't want to trigger an instruct
            bool queueCalculationJob = !isScopedJob;

            if (message.ApplicationProperties.ContainsKey("disableQueueCalculationJob") && message.ApplicationProperties["disableQueueCalculationJob"] != null)
            {
                queueCalculationJob = !bool.Parse(message.ApplicationProperties["disableQueueCalculationJob"].ToString());
            }
            ...
            if (!message.ApplicationProperties.ContainsKey("specification-id"))
            {
            ...
            if (!message.ApplicationProperties.ContainsKey("relationship-id"))
            {
            ...
            string relationshipId = message.ApplicationProperties["relationship-id"].ToString();
            ...
            try
            {
                BuildProject buildProject = await ProcessDataset(dataset, specification, relationshipId, relationship.Current.DatasetVersion.Version, user, relationship.Current.IsSetAsProviderData, correlationId, relationship.Current.RelationshipType);
                ...
                if (isScopedJob && specification.ProviderSource != ProviderSource.FDZ)
                {
                    IEnumerable<DefinitionSpecificationRelationship> relationships = _datasetRepository.IsConnectedToSQL()
                     ...
                     : await _datasetRepository.GetDefinitionSpecificationRelationshipsByQuery(c => c.Content.Current.Specification.Id == specificationId);
        ...
        public async Task MapFdzDatasets(ServiceBusReceivedMessage message)
        {
            Guard.ArgumentNotNull(message, nameof(message));
            ...
            { 
                ...
                foreach (DefinitionSpecificationRelationship relationship in mappedRelationships)
                {
                    ...
                    if (relationship.Current.RelationshipType == DatasetRelationshipType.FDS)
                    {
                        ...
                        ApiResponse<FDSDatasetVersion> datasetVersion = await _fdsApiClientPolicy.ExecuteAsync(() => _fdSApiClient.GetDatasetVersionsBySnapshotId(relationship.DatasetId));
                        if (datasetVersion == null || datasetVersion.Content == null)
                        {
                            string errorMessage = $"FDS API Failure: No FDS Dataset Versions found for definition id '{relationship.Current.DatasetDefinition.Id}'.";
                            _logger.Error(errorMessage);
        ...
        private async Task<BuildProject> ProcessDataset(Dataset dataset,
            ...
            DatasetRelationshipType relationshipType )
        {
            string dataDefinitionId = dataset.Definition?.Id;
            string fullBlobName = String.Empty;

            if (relationshipType != DatasetRelationshipType.FDS)
            {
                ...
                fullBlobName = datasetVersion.BlobName;
            }
            ...
            if (dataDefinitionId != null)
            {
                if (relationshipType == DatasetRelationshipType.FDS)
                {
                    ApiResponse<FDSDatasetDefinition> fdsDefinition = await _fdsApiClientPolicy.ExecuteAsync(() => _fdSApiClient.GetDatasetDefinition(dataDefinitionId));

                    if (fdsDefinition == null || fdsDefinition.Content == null)
                    {
                        string message = $"No FDS DatasetDefinition found for definition id - '{dataDefinitionId}' During ProcessDataset execution";
                ...
                {
                    datasetDefinition = _datasetRepository.IsConnectedToSQL()
            ...
            if (datasetDefinition != null)
            {
                if (relationshipType == DatasetRelationshipType.FDS)
                {
                    ApiResponse<FDSDatasourceDataModel> datasourceData = await _fdsApiClientPolicy.ExecuteAsync(() => _fdSApiClient.GetDatasourceDataBySnapshotId(dataset.Current.DatasetId));

                    if (datasourceData == null || datasourceData.Content == null)
                    {
                        string message = $"No FDS API Datasource data found for snapshot id - '{dataset.Current.DatasetId}' During ProcessDataset execution";
                        _logger.Error(message);
                        throw new NonRetriableException(message);
                    }
                    loadResult = _mapper.Map<TableLoadResult>(datasourceData.Content);
                }
                else
                {
            ...
            string fullBlobName)
        {
                ...
                KeyValuePair<string, ProviderSourceDataset> providerSourceDataset = resultsByProviderId.ElementAt(resultByProviderId);
                    ...
                    if (existingCurrent.ContainsKey(providerId))
                    {
                        ...
                        if (existingDatasetJson != latestDatasetJson || existingCurrent[providerId].Deleted)
                        {
                            ...
                            newVersion.Dataset = sourceDataset.Current.Dataset;
            ...
            if (historyToSave.Any())
            {
                ...
                if (_providersResultsRepository.IsConnectedToSQL())
                {
                    await _bulkSourceDatasetsVersionRepository.SaveVersions(historyToSave);
                }
                else
                {
