...
namespace CalculateFunding.Services.Datasets
{
    public class CalcsRepository : ICalcsRepository
    {
        ...
        public async Task<BuildProject> GetCompiledBuildProjectBySpecificationId(string specificationId)
        {
            if (string.IsNullOrWhiteSpace(specificationId))
                throw new ArgumentNullException(nameof(specificationId));           

            ApiResponse<Common.ApiClient.Calcs.Models.BuildProject> apiResponse = await _apiClientPolicy.ExecuteAsync(() => _apiClient.GetCompiledBuildProjectBySpecificationId(specificationId));
