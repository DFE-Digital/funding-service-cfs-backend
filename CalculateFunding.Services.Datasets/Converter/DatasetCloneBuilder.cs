...
namespace CalculateFunding.Services.Datasets.Converter
{
    public class DatasetCloneBuilder : IDatasetCloneBuilder
    {
            ...
            DatasetDefinition datasetDefinition)
        {
            ...
            (string blobPath, SdkBlobClient blob) = await GetExcelBlob(dataset);
                              ...
                              Stream excelStream)
        {
            ...
            SdkBlobClient blob = await Blobs.GetBlobReferenceFromServerAsync(blobPath);

            await Blobs.UploadAsync(blob, excelStream);

            var metadata = new Dictionary<string, string>
            {
                ["datadefinitionid"] = datasetDefinition.Id,
                ["datasetid"] = dataset.Id,
                ["authorid"] = author.Id,
                ["authorname"] = author.Name,
                ["name"] = dataset.Current.BlobName,
                ["description"] = dataset.Current.Description,
                ["fundingstreamid"] = datasetDefinition.FundingStreamId,
                ["converterwizard"] = true.ToString().ToLower()
            };

            await Blobs.AddMetadataAsync(blob, metadata);
        }

        private async Task<(string path, SdkBlobClient blob)> GetExcelBlob(Dataset dataset)
        {
            string blobPath = dataset.Current.BlobName;

            SdkBlobClient blob = await BlobsResilience.ExecuteAsync(() => Blobs.GetBlobReferenceFromServerAsync(blobPath));
