    ...
    public class DatasetDataMergeService : IDatasetDataMergeService
    {
            ...
            DatasetEmptyFieldEvaluationOption emptyFieldEvaluationOption)
        {
            ...
            if (result.HasChanges)
            {
                ...
                SdkBlobClient blob = await _blobClient.GetBlobReferenceFromServerAsync(blobFileNameToMerge);

                try
                {
                    await using MemoryStream memoryStream = new MemoryStream(excelAsBytes);
                    await _blobClientPolicy.ExecuteAsync(() => _blobClient.UploadAsync(blob, memoryStream));
                }
                catch (Exception ex)
                {
        ...
        private async Task<(bool success, string errorMessage, List<TableLoadResult> tableLoadResults)> ReadExcelDatasetData(DatasetDefinition datasetDefinition,
            string blobFileName)
        {
            SdkBlobClient blob = await _blobClient.GetBlobReferenceFromServerAsync(blobFileName);
