...
namespace CalculateFunding.Services.Datasets
{
    public class DefinitionsService : IDefinitionsService, IHealthChecker
    {
...
)
        {
            ...
            IEnumerable<DatasetDefinition> defintions = _datasetsRepository.IsConnectedToSQL()
                ? await _datasetsRepositoryPolicy.ExecuteAsync(() => _datasetsRepository.GetDatasetDefinitionsBySQLQuery(s => definitionIds.Contains(s.Id)))
                : await _datasetsRepositoryPolicy.ExecuteAsync(() => _datasetsRepository.GetDatasetDefinitionsByQuery(d => definitionIds.Contains(d.Id)));
        ...
        private async Task SaveToBlobStorage(byte[] excelfile, string definitionName)
        {
            ...
            SdkBlobClient blob = await _blobClient.GetBlobReferenceFromServerAsync($"schemas/{friendlyDefinitionName}.xlsx");


            try
            {
                var payload = BinaryData.FromBytes(excelfile);

                await _blobClientPolicy.ExecuteAsync(() => blob.UploadAsync(payload, overwrite: true)); 
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetDatasetSchemaSasUrl(DatasetSchemaSasUrlRequestModel requestModel)
        {
            ...
            string blobUrl = _blobClient.GetBlobSasUrl(fileName, DateTimeOffset.UtcNow.AddDays(1), BlobSasPermissions.Read);
        ...
        private async Task<IActionResult> SaveDatasetDefinition(DatasetDefinition definition, string correlationId, Reference user)
        {
            ...
            try
            {
                ...
                {
                    string errorMessage = $"Funding stream with ID '{definition.FundingStreamId}' not found for dataset definition - {definition.Name}";
                    _logger.Error(errorMessage);
                    return new BadRequestObjectResult(errorMessage);
                }
