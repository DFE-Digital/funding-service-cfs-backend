    ...
    public class CreateNewDatasetModelValidator : AbstractValidator<CreateNewDatasetModel>
    {
            ...
            IPolicyRepository policyRepository)
        {
            ...
            .CustomAsync(async (name, context, ct) =>
            {
                ...
                {
                    IEnumerable<Dataset> datasets = _datasetsRepository.IsConnectedToSQL()
                        ? await _datasetsRepository.GetDatasetsBySQLQuery(m => m.Name.ToLower() == model.Name.ToLower() && !m.IsDeleted)
                        : await _datasetsRepository.GetDatasetsByQuery(m => m.Content.Name.ToLower() == model.Name.ToLower());
