...
namespace CalculateFunding.Services.Processing
{
    public abstract class JobProcessingService : ProcessingService,  IJobProcessingService
    {
        ...
        public override async Task Run(ServiceBusReceivedMessage message, Func<Task> func = null)
        {
            ...
            if (!message.ApplicationProperties.ContainsKey(JobIdKey))
            {
        ...
        public async Task NotifyProgress(int itemCount)
        {
            ...
            await _jobManagement.AddJobLog(Job.Id, new JobLogUpdateModel { ItemsProcessed = itemCount });
        }

        public async Task NotifyPercentComplete(int percent)
        {
            if (Job == null)
            {
                return;
            ...
            await _jobManagement.UpdateJobStatus(Job.Id, percent, null, null);
        }
