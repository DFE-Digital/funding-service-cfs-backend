...
namespace CalculateFunding.Services.Processing.Functions
{
    public abstract class SmokeTest
    {
        ...
        private bool IsSmokeTest(ServiceBusReceivedMessage message) => message.ApplicationProperties.ContainsKey(SmokeTestKey);

        public string BuildNumber => FileVersionInfo.GetVersionInfo(GetType().Assembly.Location).FileVersion;

        protected virtual async Task Run(ServiceBusReceivedMessage message, Func<Task> func = null)
        {
            ...
            if (IsSmokeTest(message))
            {
                ...
                string invocationId = message.ApplicationProperties[SmokeTestKey].ToString();
