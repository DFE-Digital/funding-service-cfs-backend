...
namespace CalculateFunding.Services.Policy.TemplateBuilder
{
    public class TemplateSearchService : ITemplateSearchService, IHealthChecker
    {
        ...
        //internal use only
        public async Task RemoveTemplateFormIndexSearch(string fundingStreamId, string fundingPeriodId ,string templateId)
        {          
            string searchFilter = $"(fundingStreamId eq '{fundingStreamId}') and (fundingPeriodId eq '{fundingPeriodId}') and (id eq '{templateId}')";            
            try
            {
                SearchResults<TemplateIndex> results = await _searchRepository.Search("", new SearchParameters { Filter = searchFilter }, allResults: true);
                if (results != null || results?.Results.Count > 0)
                {
                    _logger.Information("Successfully retrieved the template records");
                    await _searchRepository.Remove(results.Results.Select(templateResult => templateResult.Result));
                    _logger.Information("Successfully removed the template records");
                }
            }
            catch (Exception ex)
            {
                _logger.Error($"An error occurred when attempting to delete template for the fundingStreamId {fundingStreamId} " +
                    $"and fundingPeriodId {fundingPeriodId} and templateId {templateId}: {ex.Message}");
                throw;
            }
