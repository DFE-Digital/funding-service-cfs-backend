...
namespace CalculateFunding.Services.Policy.TemplateBuilder
{
    public class TemplateBuilderService : ITemplateBuilderService, IHealthChecker
    {
        ...
        public async Task<ServiceHealth> IsHealthOk()
        {
            ...
            {
                return await _templateRepository.GetHealthStatus();
            }
        ...
        public async Task<CommandResult> CreateTemplate(TemplateCreateCommand command, Reference author)
        {
            ...
            try
            {
                IEnumerable<FundingStreamWithPeriods> available = await GetFundingStreamAndPeriodsWithoutTemplates(command.FundingStreamId);
                ...
                if (result.IsSuccess())
                {
                    if (enableEfCoreSqlFlag)
                    {
                        //In sql templateVersion save
                        await _templateVersionRepository.SaveTemplateVersion(template.Current);
                    }
                    else
                    {
                        //In cosmos templateVersion save
                ...
                string errorMessage = $"Failed to create a new template with name {templateName} in DB. Status code {(int)result}";
        ...
        public async Task<CommandResult> CreateTemplateAsClone(TemplateCreateAsCloneCommand command, Reference author)
        {
            ...
            try
            {
                ...
                {
                    HttpStatusCode result = await _templateRepository.CreateDraft(template);
                    HttpStatusCode templateVersionUpdateResult = await _templateVersionRepository.SaveTemplateVersion(template.Current);
                    ...
                    string errorMessage = $"Failed to create a new template with name {templateName} in Sql. Status code {(int)result}";
                    _logger.Error(errorMessage);
        ...
        public async Task<IEnumerable<FundingStreamWithPeriods>> GetFundingStreamAndPeriodsWithoutTemplates([Optional]string fundingStreamCode)
        {
            ...
            Task<IEnumerable<Template>> allTemplatesTask = _templateRepository.GetAllTemplates(fundingStreamCode);
        ...
        public async Task<CommandResult> PublishTemplate(TemplatePublishCommand command)
        {
            ...
            var templatePredecessorId = template.Current.Id;
            // create new version and save it
            ...
            if (enableEfCoreSqlFlag)
            {
                ...
                {
                    return CommandResult.Fail($"Failed to update template templateupdateresult: {templateUpdateResult}");
                }
                var templatePredecessorSaveResult = await _templateRepository.SaveTemplatePredecessor(template.TemplateId, templatePredecessorId);
                if (!templatePredecessorSaveResult.IsSuccess())
                {
                    return CommandResult.Fail($"Failed to save template predecessor: {templatePredecessorSaveResult}");
                }
                var templateSaveTemplateVersionResult = await _templateVersionRepository.SaveTemplateVersion(newVersion); // cosmos and sql same method name because its use everywhere 
                if (!templateSaveTemplateVersionResult.IsSuccess())
                {
                    return CommandResult.Fail($"Failed to template save templateversionresult: {templateSaveTemplateVersionResult}");
                }
            }
            else
            {
            ...
            int majorVersion, int minorVersion)
        {
            ...
            if (enableEfCoreSqlFlag)
            {
                ...
                await _templateRepository.SaveTemplatePredecessor(template.TemplateId, templatePredecessorId);
                var templateVersionResult = await _templateVersionRepository.SaveTemplateVersion(newVersion);
                if (templateVersionResult == HttpStatusCode.OK)
                {
                    await CreateTemplateIndexItem(template, author);
                }
        ...
        private async Task<HttpStatusCode> UpdateTemplateDescription(TemplateDescriptionUpdateCommand command, Reference author, Template template)
        {
            ...
            if (enableEfCoreSqlFlag)
            {
                template.Current.Author = author;
                HttpStatusCode updateDescriptionResult = await _templateRepository.UpdateDescription(template);
                if (updateDescriptionResult == HttpStatusCode.OK)
                {
                    await CreateTemplateIndexItem(template, author);
                }
        ...
        private bool GetEnableEfCoreSqlFlag()
        {
            ...
            _logger.Information($"Get EnableEfCoreSqlFlag Form TemplateBuilderService For TemplateVerionRepository :{efCoreSqlFlag}");
