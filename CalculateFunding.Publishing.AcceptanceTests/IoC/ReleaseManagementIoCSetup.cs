...
namespace CalculateFunding.Publishing.AcceptanceTests.IoC
{
    ...
    public class ReleaseManagementIoCSetup : SetupBase
    {
        ...
        private void RegisterRepositories()
        {
            ...
            RegisterFactoryAs<Services.Core.Interfaces.AzureStorage.IBlobClient>((svc) =>
            {
                IReleaseManagementBlobStepContext releaseManagementBlobStepContext = svc.Resolve<IReleaseManagementBlobStepContext>();
                ReleaseManagementBlobStepContext stepContext = (ReleaseManagementBlobStepContext)releaseManagementBlobStepContext;
                return stepContext.PublishedProvidersClient;
            });

            // Ensure Common.Storage.IBlobClient is available for components that expect the common blob client
            RegisterFactoryAs<CalculateFunding.Common.Storage.IBlobClient>((svc) =>
            {
                IReleaseManagementBlobStepContext releaseManagementBlobStepContext = svc.Resolve<IReleaseManagementBlobStepContext>();
                ReleaseManagementBlobStepContext stepContext = (ReleaseManagementBlobStepContext)releaseManagementBlobStepContext;
                // Use the PublishedFundingClient (InMemoryBlobClient implementing CalculateFunding.Common.Storage.IBlobClient)
                return stepContext.PublishedFundingClient;
            });
