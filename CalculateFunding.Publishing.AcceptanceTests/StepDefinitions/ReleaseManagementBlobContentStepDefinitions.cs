...
namespace CalculateFunding.Publishing.AcceptanceTests.StepDefinitions
{
    public class ReleaseManagementBlobContentStepDefinitions : StepDefinitionBase
    {
        ...
        {
            var files = _blobContext.PublishedProvidersClient.GetFiles();
            int actual = files.Count;
            actual.Should().Be(expectedTotalFiles, $"Published providers blob storage contains {actual} files: {string.Join(", ", files.Keys)}");
        }
