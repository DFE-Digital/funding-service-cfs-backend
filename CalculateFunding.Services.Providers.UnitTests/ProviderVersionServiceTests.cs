    ...
    public class ProviderVersionServiceTests
    {
        ...
        public async Task UploadProviderVersion_WhenAllPropertiesPopulated()
        {
            ...
            IProviderVersionService providerService = SetupAnAllPropertiesPopulatedProviderVersionService(
                ProviderVersionId, Description, providerVersionViewModel, searchRepository, blobClient, providerVersionsMetadataRepository);
            ...
            // ensure blob client was asked to get a blob reference for upload
            await blobClient
                .Received(1)
        ...
        private IProviderVersionService SetupAnAllPropertiesPopulatedProviderVersionService(
            ...
            ISearchRepository<ProvidersIndex> searchRepository, IBlobClient blobClient, IProviderVersionsMetadataRepository providerVersionsMetadataRepository)
        {
            const string id = "1";
            ...
            // no need to setup GetBlockBlobReference for new implementation; upload uses GetBlobReferenceFromServerAsync
        ...
        public async Task GetAllProviders_WhenProviderVersionIdExistsAndFileSystemCachingEnabled_ProviderListReturned_DocumentCached()
        {
            ...
            // simulate download returning a stream
            blobClient
                .DownloadToStreamAsync(Arg.Any<BlobBaseClient>())
                .Returns(Task.FromResult((Stream)memoryStream));
        ...
        private IBlobClient CreateBlobClient()
        {
            ...
            // Ensure UploadAsync returns a completed task even when blob reference is null in tests.
            blobClient.UploadAsync(Arg.Any<Azure.Storage.Blobs.BlobClient>(), Arg.Any<Stream>())
                .Returns(Task.CompletedTask);

            blobClient.UploadAsync(Arg.Any<Azure.Storage.Blobs.BlobClient>(), Arg.Any<string>())
                .Returns(Task.CompletedTask);

            // Ensure GetBlobReferenceFromServerAsync returns a non-null substitute blob client to avoid null reference
            // when production code dereferences the returned blob client.
            blobClient.GetBlobReferenceFromServerAsync(Arg.Any<string>())
                .Returns(ci => Task.FromResult(Substitute.For<Azure.Storage.Blobs.BlobClient>()));
