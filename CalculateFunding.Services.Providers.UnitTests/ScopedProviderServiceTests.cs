...
namespace CalculateFunding.Services.Providers.UnitTests
{
    ...
    public class ScopedProviderServiceTests
    {
        ...
        {
            IDictionary<string, object> newProperties = properties.ToDictionary(p => p.key, p => (object)p.value);

            if (_message?.ApplicationProperties != null)
            {
                foreach (var kv in _message.ApplicationProperties)
                {
                    if (!newProperties.ContainsKey(kv.Key))
                    {
            ...
            _message = ServiceBusModelFactory.ServiceBusReceivedMessage(
                ...
                messageId: _message?.MessageId ?? Guid.NewGuid().ToString());
        }
