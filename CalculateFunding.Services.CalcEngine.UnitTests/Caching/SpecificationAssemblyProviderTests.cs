...
// Aliases to disambiguate between project-specific Blob/IBlobClient and Azure SDK types
...
namespace CalculateFunding.Services.CalcEngine.Caching
{
    ...
    public class SpecificationAssemblyProviderTests
    {
        ...
        private Mock<CoreBlobClient> _blobs;
        ...
        public void SetUp()
        {
            ...
            _blobs = new Mock<CoreBlobClient>();
            ...
            Stream assembly)
        {
            ...
            var uri = new Uri($"http://localhost/{Guid.NewGuid()}/{blobName}");
            SdkBlobClient blobClient = new SdkBlobClient(uri);

            _blobs
                .Setup(_ => _.GetBlobReferenceFromServerAsync(blobName))
                .ReturnsAsync(blobClient);
        ...
        private void AndTheBlobPropertiesWereFetched()
            => _blobs.Verify(_ => _.GetBlobETagAsync(It.IsAny<SdkBlobClient>()), Times.Once);

        private void AndTheBlobHasTheEtag(string etag)
        {
            _blobs.Setup(_ => _.GetBlobETagAsync(It.IsAny<SdkBlobClient>())).ReturnsAsync(etag);
        }
