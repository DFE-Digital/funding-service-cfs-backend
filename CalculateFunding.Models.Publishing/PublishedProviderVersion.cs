...
namespace CalculateFunding.Models.Publishing
{
    ...
    public class PublishedProviderVersion : VersionedItem
    {
        ...
        /// <summary>
        /// The ReProfiledOnDemand Profile periods used for this provider
        /// in this period and funding stream keyed by funding line
        /// </summary>
        [JsonProperty("reProfiledOnDemandProfiles")]
        public IEnumerable<FundingLineOnDemandReProfiles> ReProfiledOnDemandProfiles { get; set; }

        /// <summary>
        /// Flag indicating whether this provider has any ReProfiledOnDemand profiles 
        /// </summary>
        [JsonProperty("hasReProfiledOnDemandProfiles")]
        public bool HasReProfiledOnDemandProfiles => ReProfiledOnDemandProfiles?.Any() == true;
        ...
        public bool IsConverter(DateTimeOffset? fundingPeriodStartDate,
            ...
            bool skipPredecessorsCheck = false) 
        {
            return Provider != null && 
                Provider.ReasonEstablishmentOpened == academyConverter &&
                //skipPredecessorsCheck - flag to skip the predecessors check for the converters when reProfiledOnDemand is applied
                (Provider.Predecessors.AnyWithNullCheck() || skipPredecessorsCheck) &&
        ...
        public void AddReprofiledOnDemandProfile(string fundingLineCode)
        {
            ReProfiledOnDemandProfiles ??= ArraySegment<FundingLineOnDemandReProfiles>.Empty;

            FundingLineOnDemandReProfiles onDemandReProfile = ReProfiledOnDemandProfiles.SingleOrDefault(_ =>
                _.FundingLineCode == fundingLineCode);

            if (onDemandReProfile == null)
            {
                onDemandReProfile = new FundingLineOnDemandReProfiles
                {
                    FundingLineCode = fundingLineCode
                };

                ReProfiledOnDemandProfiles = ReProfiledOnDemandProfiles.Concat(new[]
                {
                    onDemandReProfile
                });
            }
