    ...
    public class ProvidersFunctions : SmokeTestBase
    {
        ...
        public async Task OnPopulateScopedProvidersEvent_SmokeTestSucceeds()
        {
            ...
            SmokeResponse response = await RunSmokeTest(ServiceBusConstants.QueueNames.PopulateScopedProviders,
                async(ServiceBusReceivedMessage smokeResponse) => await onPopulateScopedProvidersEventTrigger.Run(JsonConvert.DeserializeObject<ServiceBusReceivedMessage>(Encoding.UTF8.GetString(smokeResponse.Body))));
        ...
        public async Task OnProviderSnapshotDataLoadEvent_SmokeTestSucceeds()
        {
            ...
            SmokeResponse response = await RunSmokeTest(ServiceBusConstants.QueueNames.ProviderSnapshotDataLoad,
                async(ServiceBusReceivedMessage smokeResponse) => await onProviderSnapshotDataLoadEventTrigger.Run(JsonConvert.DeserializeObject<ServiceBusReceivedMessage>(Encoding.UTF8.GetString(smokeResponse.Body))), useSession: true);
