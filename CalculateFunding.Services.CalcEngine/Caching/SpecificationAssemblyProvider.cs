...
using Polly;
using Serilog;
// Use alias to resolve ambiguity between IBlobClient interfaces
using CoreBlobClient = CalculateFunding.Services.Core.Interfaces.AzureStorage.IBlobClient;
...
namespace CalculateFunding.Services.CalcEngine.Caching
{
    public class SpecificationAssemblyProvider : ISpecificationAssemblyProvider
    {
        ...
        public async Task SetAssembly(string specificationId, Stream assembly)
        {
            ...
            SdkBlobClient assemblyBlob = await GetAssemblyBlobClient(specificationId);
            
            Guard.ArgumentNotNull(assemblyBlob, nameof(assemblyBlob));

            string blobEtag = await _blobs.GetBlobETagAsync(assemblyBlob);

            _fileSystemCache.Add(GetFileSystemCacheKey(specificationId, blobEtag), assembly, ensureFolderExists: true);
        }
        ...
        private async Task<(Stream assembly, string etag)> GetAssemblyFromBlobStorage(string specificationId)
        {
            ...
            string eTag = await _blobs.GetBlobETagAsync(assemblyBlob);

            return (assembly, eTag);
        }

        private Task<SdkBlobClient> GetAssemblyBlobClient(string specificationId)
        {
