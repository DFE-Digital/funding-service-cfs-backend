...
namespace CalculateFunding.Services.CalcEngine
{
    public class AllocationModel : IAllocationModel
    {
        ...
        public Type DatasetType(string datasetName)
        {
            if(_datasetTypes.ContainsKey(datasetName))
            {
               return _datasetTypes[datasetName];
            }
            var datasetTypes = JsonSerializer.Serialize(_datasetTypes.Keys);
            string error = $"Unable to find Dataset Types:'{datasetTypes}' for Datasetname : '{datasetName}'.";
            _logger.Error(error);
        ...
        private void SetInstanceAggregationsValues(object instance, IEnumerable<CalculationAggregation> aggregationValues)
        {
            ...
            _logger.Information("Aggregations count - {count}" + aggregationValues.Count());
                ...
                if (fieldAttribute != null)
                {
                    ...
                    if (row.TryGetValue(propertyName, out object value))
                    {
                        ...
                        if (propType == "System.DateTime")
                        {
                            if (value != null)
                            {                              
                                bool isValidDate = DateTime.TryParseExact(value.ToString(), new string[] { "dd/MM/yyyy", "dd/MM/yyyy HH:mm:ss" }, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime parsedDate);
                                if (isValidDate)
                                {
                                    value = parsedDate;
                                }
