...
namespace CalculateFunding.Services.CalcEngine
{
    public class CalculationEngineServiceValidator : ICalculationEngineServiceValidator
    {
        ...
        public void ValidateMessage(ILogger logger, ServiceBusReceivedMessage message)
        {
            if (!message.ApplicationProperties.ContainsKey("provider-summaries-partition-index"))
            {
            ...
            if (!message.ApplicationProperties.ContainsKey("provider-summaries-partition-size"))
            {
            ...
            if (!message.ApplicationProperties.ContainsKey("provider-cache-key"))
            {
            ...
            int partitionSize = int.Parse(message.ApplicationProperties["provider-summaries-partition-size"].ToString());
