...
namespace CalculateFunding.Services.Compiler
{
    public static class SourceCodeHelpers
    {
        ...
        public static IEnumerable<string> GetCalculationAggregateFunctionParameters(string sourceCode)
        {
            ...
            IEnumerable<string> aggregateParameters = GetAggregateFunctionParameter(sourceCode, false);
        ...
        public static IEnumerable<string> GetDatasetAggregateFunctionParameters(string sourceCode)
        {
            ...
            IEnumerable<string> aggregateParameters = GetAggregateFunctionParameter(sourceCode, true);
        ...
        private static IEnumerable<string> GetAggregateFunctionParameter(string sourceCode, bool isDatasetAggregation)
        {
            MatchCollection matchCollection = isDatasetAggregation 
                ? Regex.Matches(sourceCode, @"(\b((Min|Max|Avg|Sum)[\s]*\()([a-zA-Z0-9_.])+?\))(\b|\s|$)", RegexOptions.IgnoreCase)
                : Regex.Matches(sourceCode, @"(\b((Min|Max|Avg|Sum)[\s]*\()([a-zA-Z0-9_.])+)(\b|\s|$)", RegexOptions.IgnoreCase);
