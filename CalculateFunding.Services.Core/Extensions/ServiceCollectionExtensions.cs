...
namespace CalculateFunding.Services.Core.Extensions
{
    public static class ServiceCollectionExtensions
    {
        ...
        public static IServiceCollection AddServiceBus(this IServiceCollection builder, IConfiguration config, string serviceName = null)
        {
            if (Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == "Development")
            {
                builder
                    ...
                    {
                        Common.ServiceBus.StorageQueueManager queueClient = new Common.ServiceBus.StorageQueueManager("UseDevelopmentStorage=true");
            ...
            else
            {
                ...
                // If no connection string provided in configuration treat as development (use storage queues)
        ...
        {           
            ...
            builder.AddSingleton<HeaderTelemetryInitializer>();

            builder.AddHttpContextAccessor();

            builder.AddSingleton<ITelemetryInitializer>(provider =>
            {
                var httpContextAccessor = provider.GetRequiredService<IHttpContextAccessor>();
                return new HeaderTelemetryInitializer(httpContextAccessor)
                {
                    RequestHeaders = new List<string> { "Accept" , "x-ms-request-change"},
                    ResponseHeaders = new List<string> { "Content-Type", "x-ms-request-change" }
                };
            });
