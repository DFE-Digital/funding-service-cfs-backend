...
namespace CalculateFunding.Services.Core.Extensions
{
    ...
    {
        public static Reference GetUserDetails(this ServiceBusReceivedMessage message)
        {
        ...
        public static UserProfile GetUserProfile(this ServiceBusReceivedMessage message)
        {
            return new UserProfile(message.GetUserDetails());
        }

        public static string GetOperationId(this ServiceBusReceivedMessage message)
        {
            string operationId = null;
            if (message.ApplicationProperties.ContainsKey("operation-id"))
            {
                operationId = message.ApplicationProperties["operation-id"]?.ToString();
            }
            return operationId;
        }

        public static T GetPayloadAsInstanceOf<T>(this ServiceBusReceivedMessage message)
        {
        ...
        public static IDictionary<string, string> BuildMessageProperties(this ServiceBusReceivedMessage message)
        {
        ...
        public static string GetMessageBodyStringFromMessage(ServiceBusReceivedMessage message)
        {
            string json = "";

            if (message.ContentType == "application/gzip")
            {
                using MemoryStream inputStream = new MemoryStream(message.Body.ToArray());
        ...
        public static T GetUserProperty<T>(this ServiceBusReceivedMessage message, string key) 
        {
            return (!message.ApplicationProperties.ContainsKey(key) || message.ApplicationProperties[key] == null)
                ? default
                : (T)message.ApplicationProperties[key];
        }
    }
}
