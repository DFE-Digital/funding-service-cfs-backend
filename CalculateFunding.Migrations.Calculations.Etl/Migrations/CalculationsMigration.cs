...
namespace CalculateFunding.Migrations.Calculations.Etl.Migrations
{
    public class CalculationsMigration
    {
        ...
        public async Task Run(string sourceSpecificationId, string destinationSpecificationId)
        {
            ...
            WriteLine("Checking source and destination specifications have build calculations successfully");

            if (HaveUnSuccessfulBuildCalculationsInSpecification(sourceSpecificationId, destinationSpecificationId))
            {
                PressAnyKeyToExit();
        ...
        private bool HaveUnSuccessfulBuildCalculationsInSpecification(string sourceSpecificationId, string destinationSpecificationId)
        {
            Task<ApiResponse<BuildProject>> sourceCalculationsBuildProjectTask = _sourceClients.MakeCalculationsCall(
                _ => _.GetCompiledBuildProjectBySpecificationId(sourceSpecificationId));
            Task<ApiResponse<BuildProject>> destinationCalculationsBuildProjectTask = _destinationClients.MakeCalculationsCall(
                _ => _.GetCompiledBuildProjectBySpecificationId(destinationSpecificationId));


            if (sourceCalculationsBuildProjectTask.Result?.Content?.Build == null || !sourceCalculationsBuildProjectTask.Result.Content.Build.Success)
            {
                WriteLine("Unable to build calculations in the source specification. Please make sure the calcs are build in the source specification for migration");
                return true;
            }

            if (destinationCalculationsBuildProjectTask.Result?.Content?.Build == null || !destinationCalculationsBuildProjectTask.Result.Content.Build.Success)
            {
                WriteLine("Unable to build calculations in the destination specification. Please make sure the calcs are build in the destination specification for migration");
                return true;
            }
        ...
        private class DataSourceNameAndSchemaComparer : IEqualityComparer<DatasetSpecificationRelationshipViewModel>
        {
            ...
            public int GetHashCode(DatasetSpecificationRelationshipViewModel obj)
            {
                unchecked
                {
                    ...
                    if(obj.RelationshipType == Common.ApiClient.DataSets.Models.DatasetRelationshipType.FDS)
                    {
                        hashCode = hashCode * 297 + (obj.Definition?.Name.Replace("_FDS" + obj.Definition?.Id, String.Empty).GetHashCode()).GetValueOrDefault();
                    }
                    else
                    {
