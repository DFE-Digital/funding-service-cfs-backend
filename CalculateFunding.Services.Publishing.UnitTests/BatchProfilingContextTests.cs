...
namespace CalculateFunding.Services.Publishing.UnitTests
{
    ...
    public class BatchProfilingContextTests
    {
        ...
        private static IEnumerable<object[]> ReconcileResponseExamples()
        {
            ...
            string reasonEstablishmentOpened = nameof(reasonEstablishmentOpened);
            DateTimeOffset date = DateTimeOffset.UtcNow;
            string dateOpened = date.ToString();
            decimal fundingValue = 111.0M;

            string key = $"{fundingPeriodId}-{fundingStreamId}-{profilePatternKey}-{providerType}-{providerSubType}-{dateOpened}-{reasonEstablishmentOpened}-{fundingLineCode}-{fundingValue:N4}";
            ...
            ProfilingBatch profilingBatchOne = NewProfilingBatch(_ => _.WithProviderType(providerType)
                ...
                .WithDateOpened(date)
                .WithReasonEstablishementOpened(reasonEstablishmentOpened)
