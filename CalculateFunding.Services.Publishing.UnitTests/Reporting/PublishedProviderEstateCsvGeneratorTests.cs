...
namespace CalculateFunding.Services.Publishing.UnitTests.Reporting
{
    ...
    public class PublishedProviderEstateCsvGeneratorTests 
    {
        ...
        // Use the Microsoft.Azure.Storage.Blob.BlobProperties type because ICloudBlob.Properties is of that type
        private Microsoft.Azure.Storage.Blob.BlobProperties _blobProperties;
        ...
        public void SetUp()
        {
            ...
            // instantiate the Microsoft.Azure.Storage.Blob.BlobProperties so it matches ICloudBlob.Properties
            _blobProperties = new Microsoft.Azure.Storage.Blob.BlobProperties();
            _cloudBlob.Setup(x => x.FetchAttributesAsync(It.IsAny<CancellationToken>()))
                .Returns(Task.CompletedTask);

            // Ensure the mocked ICloudBlob has a non-null Properties instance so code can set ContentDisposition
            _cloudBlob.SetupGet(x => x.Properties).Returns(_blobProperties);
