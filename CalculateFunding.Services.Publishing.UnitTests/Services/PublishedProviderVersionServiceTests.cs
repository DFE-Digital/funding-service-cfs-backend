...
namespace CalculateFunding.Services.Publishing.UnitTests.Services
{
    ...
    public class PublishedProviderVersionServiceTests
    {
        ...
        public async Task GetPublishedProviderVersionBody_GivenReturnsFromBlobStorage_returnsOK()
        {
            ...
            Azure.Storage.Blobs.BlobClient cloudBlob = CreateBlob();
        ...
        {
            _blobClient
                .When(x => x.UploadAsync(Arg.Any<Azure.Storage.Blobs.BlobClient>(), Arg.Is(body)))
                .Do(x => { throw new Exception("Failed to upload blob"); });

            // Ensure a blob reference is returned so UploadAsync gets invoked
            _blobClient
                .GetBlobReferenceFromServerAsync(Arg.Is(blobName))
                .Returns(CreateBlob());
        ...
        {
            //Arrange
            Azure.Storage.Blobs.BlobClient blob = CreateBlob();
            ...
            //Assert
            await _blobClient.Received(1).UploadAsync(Arg.Is(blob), Arg.Is(body));
            await _blobClient.Received(1).AddMetadataAsync(
                Arg.Is(blob),
        ...
        private static Azure.Storage.Blobs.BlobClient CreateBlob()
        {
            return new Azure.Storage.Blobs.BlobClient(new Uri($"http://localhost/{Guid.NewGuid()}"));
        }
    }
}