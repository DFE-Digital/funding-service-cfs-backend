...
namespace CalculateFunding.Services.Publishing.UnitTests.Variations.Strategies
{
    ...
    public class ProviderMetadataVariationStrategyTests
    {
            ...
            VariationReason[] expectedVariationReasons)
        {
            ...
            FundingVariation fundingVariation = new FundingVariation();
            FundingConfiguration fundingConfiguration = NewFundingConfiguration(_ => _.WithFundingVariations(fundingVariation));

            await _metadataVariationStrategy.Process(variationContext, fundingConfiguration);
        ...
        [TestMethod]
        [DynamicData(nameof(RestrictedVariationReasonExamples), DynamicDataSourceType.Method)]
        public async Task TracksConfiguredPropertiesForRestrictedVariationReasons(Action<Provider> differences,
            VariationReason[] expectedVariationReasons)
        {
            ProviderVariationContext variationContext = GivenTheVariationContextHasTheCurrentAndPriorStateDifferences(differences);
        ...
        private static IEnumerable<object[]> RestrictedVariationReasonExamples()
        {
            yield return VariationExample(_ => _.Authority = NewRandomString(), VariationReason.AuthorityFieldUpdated);
            yield return VariationExample(_ => _.URN = NewRandomString(), VariationReason.URNFieldUpdated);
            yield return VariationExample(_ => _.DfeEstablishmentNumber = NewRandomString(), VariationReason.DfeEstablishmentNumberFieldUpdated);
