...
namespace CalculateFunding.Services.Publishing.UnitTests.Variations.Strategies
{
    ...
    [TestClass]
    public class ClosedProviderReProfilingVariationStrategyTests : ReProfilingVariationStrategyTestsBase
    {
        ...
        public void SetUp()
        {
            VariationStrategy = new ClosedProviderReProfilingVariationStrategy();
        }

        [TestMethod]
        public async Task FailsPreconditionCheckIfIsNotAClosedProvider()
        {
            GivenTheOtherwiseValidVariationContext();
        ...
        public async Task FailsPreconditionCheckIfThereAreNoPaidPeriods()
        {
            ...
            GivenTheOtherwiseValidVariationContext(_ => _.UpdatedProvider.Status = Closed);
            VariationContext.PriorState.Provider.Status = Closed;
            AndTheRefreshStateFundingLines(NewFundingLine(),
                NewFundingLine(_ => _.WithFundingLineCode(FundingLineCode)
                    .WithFundingLineType(FundingLineType.Payment)
                    .WithValue(NewRandomNumber())
        ...
        [TestMethod]
        public async Task TracksFundingLinesAsAffectedFundingLineCodesAndQueuesClosedProviderReProfileVariationChangeIfProviderClosed()
        {
            int year = NewRandomNumber();
            string month = NewRandomMonth();

            GivenTheOtherwiseValidVariationContext(_ => {
                _.UpdatedProvider.Status = Closed;
            });
            VariationContext.PriorState.Provider.Status = Closed;
            AndTheRefreshStateFundingLines(NewFundingLine(),
                NewFundingLine(_ => _.WithFundingLineCode(FundingLineCode)
            ...
            ThenTheVariationChangeWasQueued<ClosedProviderReProfileVariationChange>();
            AndTheAffectedFundingLinesWereTracked(FundingLineCode);
        }
    ...
    public class ReProfilingVariationStrategyTests : ReProfilingVariationStrategyTestsBase
    {
        ...
        [TestMethod]
        public async Task WillSkipAffectedFundingLineCodesOfNonZeroFundingValueTotalForIndicativeToLiveProviderAndQueuesReProfileVariationChangeIfProfilingDiffers()
        {
            int year = NewRandomNumber();
            string month = NewRandomMonth();
            ...
            AndTheAffectedFundingLinesAreNotTracked(FundingLineCode);
        }
    }
}