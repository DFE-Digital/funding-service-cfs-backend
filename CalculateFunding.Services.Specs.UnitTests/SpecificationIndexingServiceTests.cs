...
namespace CalculateFunding.Services.Specs.UnitTests
{
    ...
    public class SpecificationIndexingServiceTests
    {
        ...
        {
            ServiceBusReceivedMessage message = NewMessage(("specification-id", NewRandomString()),
        ...
        private async Task WhenTheIndexingJobIsRun(ServiceBusReceivedMessage message)
        {
        ...
        private static ServiceBusReceivedMessage NewMessage(params (string, string)[] userProperties)
        {
            IDictionary<string, object> merged =  new Dictionary<string, object>();

            foreach ((string key, object value) in userProperties)
            {
                merged[key] = value;
            }
