...
namespace CalculateFunding.Services.Results.UnitTests
{
    ...
    public class ProviderResultsCsvGeneratorServiceTests
    {
        ...
        public void SetUp()
        {
            ...
            _cloudBlob = new BlobClient(new Uri($"http://localhost/{Guid.NewGuid()}"));
            ...
            _blobProperties = null; // not required for v12 BlobClient in these tests
        }
        ...
        public async Task TransformsProviderResultsForSpecificationInBatchesAndCreatesCsvWithResults()
        {
            ...
            await _blobClient
                .Received(1)
                .UploadAsync(Arg.Is<BlobClient>(_ => _.Uri == _cloudBlob.Uri), Arg.Is<Stream>(_ => _ == incrementalFileStream));
            ...
            await _blobClient
                .Received(1)
                .AddMetadataAsync(
                    Arg.Is<BlobClient>(_ => _.Uri == _cloudBlob.Uri),
        ...
        {
            _blobClient
                .GetBlobReferenceFromServerAsync($"calculation-results-{specificationId}.csv")
                .Returns(_cloudBlob);
        }
        ...
        private void GivenTheMessageProperties(params (string,string)[] properties)
        {
            ...
            // Create a new message instance with the supplied properties
            _message = ServiceBusModelFactory.ServiceBusReceivedMessage(
